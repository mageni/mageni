...
if (description)
{
  ...
  script_version("2021-03-17T08:55:31+0000");
  ...
  script_tag(name:"summary", value:"PHP is prone to a symlink attack vulnerability in
  the included PEAR installer.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the
  target host.");

  script_tag(name:"insight", value:"The PEAR_REST class in REST.php in PEAR allows local
  users to write to arbitrary files via a symlink attack on a (1) rest.cachefile or (2)
  rest.cacheid file in /tmp/pear/cache/, related to the retrieveCacheFirst and
  useLocalCache functions.");

  script_tag(name:"affected", value:"PHP through 5.6.0.");

  script_tag(name:"solution", value:"Update to a later PHP version including an PEAR installer
  in version 1.9.2 or later.");
