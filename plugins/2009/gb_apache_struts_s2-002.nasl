...
if(description)
{
  ...
  script_name("Apache Struts XSS Vulnerability (S2-002)");
  ...
  script_tag(name:"insight", value:"This flaw is due to improper sanitization of the user
  supplied input in '<s:url>' and '<s:a>' tag which doesn't encode the URL parameter when
  specified in the action attribute which causes XSS attacks.");

  script_tag(name:"impact", value:"Successful exploitation will let the attacker issue
  malicious URL or can inject malicious codes inside the web page contents to gain
