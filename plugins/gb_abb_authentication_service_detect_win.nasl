...
if(description)
{
  ...
  script_name("ABB Authentication Service Version Detection (Windows)");
...
foreach key (key_list) {
  foreach item (registry_enum_keys(key:key)) {
    ...
    if(!appName || appName !~ "ABB Authentication Service") continue;
    ...
    set_kb_item(name:"abb/authentication_service/win/detected", value:TRUE);

    register_and_report_cpe(app:appName, ver:version, concluded:concluded,
                          base:"cpe:/a:abb:authentication_service:", expr:"^([0-9.]+)", insloc:location, regService:"smb-login", regPort:0);
    exit(0);
  }
}
