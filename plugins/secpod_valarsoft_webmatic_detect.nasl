...
if(description)
{
  ...
  script_tag(name:"summary", value:"This script finds the running Valarsoft Webmatic version and
...
foreach dir( make_list_unique( "/", "/webmatic", "/web", cgi_dirs( port:port ) ) ) {
  ...
  if( rcvRes =~ "HTTP/1.. 200" && "Welcome to Webmatic" >< rcvRes ) {

    version = "unknown";

    ver = eregmatch( pattern:"Webmatic ([0-9.]+)", string:rcvRes );
    if( ver[1] != NULL ) version = ver[1];

    tmp_version = version + " under " + install;
    set_kb_item( name:"www/" + port + "/Valarsoft/Webmatic", value:tmp_version );
    set_kb_item( name:"valarsoft/webmatic/detected", value:TRUE );
