...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150592");
  ...
  script_name("SYS.2.3.A1");

  script_category(ACT_GATHER_INFO);
  script_family("IT-Grundschutz");
  script_dependencies("os_detection.nasl", "compliance_tests.nasl",
    "Policy/Linux/AccesAndAuth/sshd_permitrootlogin.nasl");
  ...
  script_tag(name:"summary", value:"Administratoren DUERFEN sich NICHT im Normalbetrieb als 'root'
anmelden. Fuer die Systemadministrationsaufgaben SOLLTE 'sudo' oder eine geeignete Alternative mit
einer geeigneten Protokollierung genutzt werden. Es SOLLTE verhindert werden, dass sich mehrere
Benutzer auf einem Client gleichzeitig einloggen koennen.");
...
title = "Authentisierung von Administratoren und Benutzern";

desc = "Folgende Einstellungen werden getestet:
SSH login fuer root user ist deaktiviert.";

oid_list = make_list("1.3.6.1.4.1.25623.1.0.150072");

if (host_runs("linux") != "yes"){
  result = itg_result_wrong_target();
  desc = itg_desc_wrong_target();
  itg_set_kb_entries(result:result, desc:desc, title:title, id:"SYS.2.3.A1");
  exit(0);
}
