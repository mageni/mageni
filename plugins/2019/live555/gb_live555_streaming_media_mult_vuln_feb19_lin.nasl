...
if (description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.112524");
  ...
  script_name("Live555 Streaming Media < 2019.02.27 Multiple Vulnerabilities (Linux)");
  ...
  script_mandatory_keys("live555_streaming_media/installed", "Host/runs_unixoide");
  ...
  - A setup packet can cause a memory leak leading to DoS because, when there are
    multiple instances of a single filed, only the last instance can be freed
