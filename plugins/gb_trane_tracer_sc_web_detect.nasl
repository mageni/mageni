...
# $Id: gb_trane_tracer_sc_web_detect.nasl 11885 2018-10-12 13:47:20Z cfischer $
#
...
res = http_get_cache(port: port, item: "/hui/index.html");

if ("<title>Tracer SC</title>" >< res && 'setCookie("cookiecheck", "TracerSC"' >< res) {
  version = "unknown";

  req = http_get(port: port, item: "/evox/about");
  res = http_keepalive_send_recv(port: port, data: req);

  vers = eregmatch(pattern: 'productVersion" val="v([0-9.]+)', string: res);
  if (!isnull(vers[1])) {
    version = vers[1];
    set_kb_item(name: "trane_tracer/version", value: version);
    concUrl = "/evox/about";
  }

  set_kb_item(name: "trane_tracer/detected", value: TRUE);

  cpe = build_cpe(value: version, exp: "^([0-9.]+)", base: "cpe:/a:trane:tracer_sc:");
  if (!cpe)
    cpe = 'cpe:/a:trane:tracer_sc';

  register_product(cpe: cpe, location: "/", port: port, service: "www");

  log_message(data: build_detection_report(app: "Trane Tracer SC", version: version, install: "/", cpe: cpe,
