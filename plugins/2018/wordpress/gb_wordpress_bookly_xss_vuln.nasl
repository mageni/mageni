...
res = http_get_cache(port: port, item: dir + "/wp-content/plugins/bookly-responsive-appointment-booking-tool/readme.txt");

if ("=== Bookly #1 WordPress Booking Plugin" >< res && "Changelog" >< res) {

  vers = eregmatch(pattern: "Stable tag: ([0-9.]+)", string: res);

  if (!isnull(vers[1]) && version_is_less(version: vers[1], test_version: "14.5")) {
    report = report_fixed_ver(installed_version: vers[1], fixed_version: "14.5");
