...
# $Id: gb_nmap_pgsql_brute_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104138");
  ...
  script_name("Nmap NSE net: pgsql-brute");
  ...
  script_add_preference(name:"pgsql.nossl", value:"", type:"entry");
  script_add_preference(name:"pgsql.version", value:"", type:"entry");
  ...
  script_tag(name:"summary", value:"Performs password guessing against PostgreSQL.

SYNTAX:

pgsql.nossl:  If set to '1' or 'true', disables SSL.

pgsql.version:  Force protocol version 2 or 3.
...
if (phase == 1) {
    ...
    pref = script_get_preference("pgsql.nossl");
    if (!isnull(pref) && pref != "") {
        argv["pgsql.nossl"] = string('"', pref, '"');
    }
    pref = script_get_preference("pgsql.version");
    if (!isnull(pref) && pref != "") {
        argv["pgsql.version"] = string('"', pref, '"');
    }
    ...
    nmap_nse_register(script:"pgsql-brute", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"pgsql-brute");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (pgsql-brute.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
