...
# $Id: gb_nmap_mysql_users_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104050");
  ...
  script_name("Nmap NSE net: mysql-users");
  ...
  script_tag(name:"summary", value:"Attempts to list all users on a MySQL server.
...
if (phase == 1) {
    ...
    nmap_nse_register(script:"mysql-users", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"mysql-users");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (mysql-users.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
