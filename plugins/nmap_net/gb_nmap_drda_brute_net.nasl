...
# $Id: gb_nmap_drda_brute_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104073");
  ...
  script_name("Nmap NSE net: drda-brute");
  ...
  script_add_preference(name:"drda-brute.threads", value:"", type:"entry");
  script_add_preference(name:"unpwdb.userlimit", value:"", type:"entry");
  script_add_preference(name:"unpwdb.passlimit", value:"", type:"entry");
  script_add_preference(name:"drda-brute.dbname", value:"", type:"entry");
  script_add_preference(name:"unpwdb.timelimit", value:"", type:"entry");
  script_add_preference(name:"passdb", value:"", type:"entry");

  script_tag(name:"summary", value:"Performs password guessing against databases supporting the IBM DB2 protocol such as Informix, DB2
and Derby
...
drda-brute.threads:  the amount of accounts to attempt to brute
...
drda-brute.dbname:  the database name against which to guess
...
if (phase == 1) {
    ...
    pref = script_get_preference("drda-brute.threads");
    if (!isnull(pref) && pref != "") {
        argv["drda-brute.threads"] = string('"', pref, '"');
    }
    ...
    pref = script_get_preference("drda-brute.dbname");
    if (!isnull(pref) && pref != "") {
        argv["drda-brute.dbname"] = string('"', pref, '"');
    }
    ...
    nmap_nse_register(script:"drda-brute", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"drda-brute");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (drda-brute.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
