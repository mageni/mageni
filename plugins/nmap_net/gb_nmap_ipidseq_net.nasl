...
# $Id: gb_nmap_ipidseq_net.nasl 11527 2018-09-21 15:59:27Z cfischer $
#
...
if(description)
{
  ...
  script_add_preference(name:"probeport", value:"", type:"entry");
...
if (phase == 1) {
    ...
    pref = script_get_preference("probeport");
    if (!isnull(pref) && pref != "") {
        argv["probeport"] = string('"', pref, '"');
    }
    nmap_nse_register(script:"ipidseq", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"ipidseq");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (ipidseq.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
