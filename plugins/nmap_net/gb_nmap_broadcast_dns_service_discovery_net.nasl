...
# $Id: gb_nmap_broadcast_dns_service_discovery_net.nasl 11527 2018-09-21 15:59:27Z cfischer $
#
...
if(description)
{
  ...
  script_add_preference(name:"dnssd.services", value:"", type:"entry");
...
if (phase == 1) {
    ...
    pref = script_get_preference("dnssd.services");
    if (!isnull(pref) && pref != "") {
        argv["dnssd.services"] = string('"', pref, '"');
    }
    nmap_nse_register(script:"broadcast-dns-service-discovery", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"broadcast-dns-service-discovery");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (broadcast-dns-service-discovery.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
