...
# $Id: gb_ibm_websphere_portal_xss_vuln.nasl 12149 2018-10-29 10:48:30Z asteins $
#
...
if (description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.106200");
...
cookie-based authentication credentials.");
  ...
  script_tag(name:"affected", value:"WebSphere Portal 8.5, 8.0, 7 and 6.1");

  script_tag(name:"solution", value:"Check the vendor's advisory for sulutions.");

  script_xref(name:"URL", value:"https://www-01.ibm.com/support/docview.wss?uid=swg21986461");
...
if (version =~ "^8\.5\.0") {
  if (version_is_less(version: version, test_version: "8.5.0.0.10")) {
    report = report_fixed_ver(installed_version: version, fixed_version: "8.5.0.0 CF10");
...
if (version =~ "^8\.0\.0") {
  if (version_is_less(version: version, test_version: "8.0.0.1.21")) {
    report = report_fixed_ver(installed_version: version, fixed_version: "8.0.0.1 CF21");
    security_message(port: port, data: report);
    exit(0);
  }
}
...
if (version =~ "^7\.0\.0") {
  if (version_is_less(version: version, test_version: "7.0.0.2.30")) {
    report = report_fixed_ver(installed_version: version, fixed_version: "7.0.0.2 CF30");
    security_message(port: port, data: report);
    exit(0);
  }
}
