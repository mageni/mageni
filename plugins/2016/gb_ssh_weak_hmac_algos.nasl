...
if( rep = check_algo( port:port, type:"client_to_server" ) )
  report = 'The following weak client-to-server MAC algorithms are supported by the remote SSH server:\n\n' + rep + '\n\n';

if( rep = check_algo( port:port, type:"server_to_client" ) )
  report += 'The following weak server-to-client MAC algorithms are supported by the remote SSH server:\n\n' + rep + '\n\n';
