...
# $Id: gb_nmap_http_date.nasl 11966 2018-10-18 13:56:21Z cfischer $
#
# Wrapper for Nmap HTTP Date NSE script
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.801610");
  ...
  script_name("Nmap NSE: HTTP Date");
  ...
  script_tag(name:"summary", value:"This script attempts to get the date from HTTP-like services.

  This is a wrapper on the Nmap Security Scanner's http-date.nse");
...
argv = make_list("nmap","--script=http-date.nse","-p",port,get_host_ip());
...
if(res)
{
  ...
  if("http-date" >< result) {
    msg = string('Result found by Nmap Security Scanner (http-date.nse) ',
                'http://nmap.org:\n\n', result);
