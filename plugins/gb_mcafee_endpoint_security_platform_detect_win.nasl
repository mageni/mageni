...
if(description)
{
  ...
  script_name("McAfee Endpoint Security Platform Version Detection (Windows)");
  ...
  of McAfee Endpoint Security Platform for Windows.");
...
foreach key (key_list) {
  foreach item (registry_enum_keys(key:key)) {
    ...
    if(!appName || appName !~ "McAfee Endpoint Security Platform") continue;
    ...
    set_kb_item(name:"mcafee/endpoint_security_platform/win/detected", value:TRUE);

    register_and_report_cpe(app:appName, ver:version, concluded:concluded,
                          base:"cpe:/a:mcafee:endpoint_security_platform:", expr:"^([0-9.]+)", insloc:location, regService:"smb-login", regPort:0);
    exit(0);
  }
}
