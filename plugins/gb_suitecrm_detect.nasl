...
# $Id: gb_suitecrm_detect.nasl 12805 2018-12-17 06:02:11Z ckuersteiner $
#
# SuiteCRM Detection
#
# Authors:
...
if (description)
{
  ...
  script_name("SuiteCRM Detection");

  script_tag(name:"summary", value:"Detection of SuiteCRM

The script sends a connection request to the server and attempts to detect the presence of SuiteCRM and to
extract its version");
...
foreach dir (make_list_unique("/suitecrm", "/SuiteCRM", "/suite", cgi_dirs(port: port))) {
  install = dir;
  if (dir == "/")
    dir = "";
  ...
  if ('alt="SuiteCRM"' >< res && 'id="admin_options">' >< res) {
    ...
    set_kb_item(name: "suitecrm/detected", value: TRUE);
    set_kb_item(name: "suitecrm/" + port + "/detected", value: TRUE);

    cpe = build_cpe(value: version, exp: "^([0-9.]+)", base: "cpe:/a:suitecrm:suitecrm:");
    if (!cpe)
