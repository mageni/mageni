...
# for valid X.509 certificates in the not too far future. GVM should
# be able to single those attacks out. It is easier to use SHA-256
# right now, than to switch to it later.
#
...
function read_and_parse_certs( cert, port ) {
  ...
  local_var cert, port;
  local_var certobj, idx, tmp, fpr, prefix, hostnames, item;
  ...
  if( log_verbosity > 1 ) {
    ...
    debug_print( "algorithm .......: ", cert_query( certobj, "algorithm-name" ), "\n" );
    debug_print( "key-size (bits) .: ", cert_query( certobj, "key-size" ), "\n" );
  }
  ...
  if( isnull( get_kb_item( prefix + "/type" ) ) ) {
    ...
    set_kb_item( name:prefix + "/key-size", value:cert_query( certobj, "key-size" ) );
