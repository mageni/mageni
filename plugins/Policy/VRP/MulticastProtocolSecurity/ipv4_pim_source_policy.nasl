...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150262");
  ...
  script_name("Huawei Data Communication: Deploying an IPv4 PIM Source Policy");

  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2020 Greenbone Networks GmbH");
  script_family("Policy");
  script_dependencies("vrp_current_configuration_section_include_multicast.nasl", "vrp_current_configuration_pim.nasl");
...
# Check procedure:
# 1. display current-configuration | section include multicast routing-enable: check for vpn-interfaces with 'multicast routing-enable' command. If no vpn-instance with 'multicast routing-enable' exist, check fails.
# 2. display current-configuration configuration pim: Check for vpn-instances without 'source-policy'. If no pim view exists, check fails.
...
cmd = "display current-configuration | section include multicast routing-enable; display current-configuration configuration pim";
title = "Deploying an IPv4 PIM Source Policy";
solution = "Deploy IPv4 PIM source policies.";
test_type = "SSH_Cmd";
default = "Enable";
