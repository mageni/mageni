...
# 1. Get all peer ipv6 addresses (if no exists, check passes)
# 2. For all peer ipv6 addresses, password and valid-ttl-hops or ebgp-max-hop must exist
...
if(get_kb_item("Policy/vrp/installed/ERROR")){
  ...
  foreach line(split(current_bgp_configuration, keep:FALSE)){
    ...
    if(ipv6_address)
      peer_ipv6 = make_list(peer_ipv6, ipv6_address[1]);
  }
  ...
  if(max_index(peer_ipv6) == 0){
    ...
    peer_ipv6 = make_list_unique(peer_ipv6);
    foreach ip (peer_ipv6){
