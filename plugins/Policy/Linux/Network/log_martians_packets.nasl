...
solution = "sysctl -w net.ipv4.conf.all.log_martians = [0,1], sysctl -w sysctl net.ipv4.conf.default.log_martians = [0,1]";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux/sysctl/ssh/ERROR")){
  ...
  if(!net_ipv4_conf_all_log_martians || !net_ipv4_conf_default_log_martians){
    value = "Error";
    compliant = "incomplete";
    comment = "Could not find setting with sysctl";
  }else{
    if(net_ipv4_conf_all_log_martians == "1" && net_ipv4_conf_default_log_martians == "1")
      value = "Enabled";
    else
      value = "Disabled";
  ...
  if(get_kb_item("Policy/linux/sysctl/conf/ERROR")){
    ...
    foreach file (files){
      ...
      grep_pattern = egrep(string:content, pattern:"(net.ipv4.conf.all.log_martians|net.ipv4.conf.default.log_martians)");
      if(grep_pattern){
