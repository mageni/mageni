...
if(get_kb_item("Policy/linux/sysctl/ssh/ERROR")){
  ...
  # sysctl -a output
  net_ipv4_conf_all_send_redirects = get_kb_item("Policy/linux/sysctl/net.ipv4.conf.all.send_redirects");
  net_ipv4_conf_default_send_redirects = get_kb_item("Policy/linux/sysctl/net.ipv4.conf.default.send_redirects");

  if(!net_ipv4_conf_all_send_redirects || !net_ipv4_conf_default_send_redirects){
    value = "Error";
    compliant = "incomplete";
    comment = "Could not find setting with sysctl";
  }else{
    if(net_ipv4_conf_all_send_redirects == "0" && net_ipv4_conf_default_send_redirects == "0")
      value = "Disabled";
    else
      value = "Enabled";
  ...
  if(get_kb_item("Policy/linux/sysctl/conf/ERROR")){
    ...
    foreach file (files){
      ...
      grep_pattern = egrep(string:content, pattern:"(net.ipv4.conf.all.send_redirects|net.ipv4.conf.default.send_redirects)");
      if(grep_pattern){
