...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.109763");
  script_version("2020-07-29T09:51:47+0000");
  script_tag(name:"last_modification", value:"2020-07-30 10:23:02 +0000 (Thu, 30 Jul 2020)");
  script_tag(name:"creation_date", value:"2019-01-24 08:47:13 +0100 (Thu, 24 Jan 2019)");
  script_tag(name:"cvss_base", value:"0.0");
  script_tag(name:"cvss_base_vector", value:"AV:L/AC:H/Au:S/C:N/I:N/A:N");
  script_tag(name:"qod", value:"97");

  script_name("Linux: Reverse path filtering");
  ...
  script_tag(name:"summary", value:"Reverse Path Filtering is a process to confirm that a receiving
packet source address is routable. If enabled, a not routable packet is dropped.
This script tests whether the Linux host is configured to drop not routable packets.");
  exit(0);
}

include("policy_functions.inc");

cmd = "sysctl net.ipv4.conf.all.rp_filter, sysctl net.ipv4.conf.default.rp_filter";
title = "Reverse path filtering";
solution = "sysctl -w nnet.ipv4.conf.all.rp_filter = [0,1], sysctl -w net.ipv4.conf.default.rp_filter = [0,1]";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux/sysctl/ssh/ERROR")){
  ...
  # sysctl -a output
  net_ipv4_conf_all_rp_filter = get_kb_item("Policy/linux/sysctl/net.ipv4.conf.all.rp_filter");
  net_ipv4_conf_default_rp_filter = get_kb_item("Policy/linux/sysctl/net.ipv4.conf.default.rp_filter");

  if(!net_ipv4_conf_all_rp_filter || !net_ipv4_conf_default_rp_filter){
    value = "Error";
    compliant = "incomplete";
    comment = "Could not find setting with sysctl";
  }else{
    if(net_ipv4_conf_all_rp_filter == "1" && net_ipv4_conf_default_rp_filter == "1")
      value = "Enabled";
    else
      value = "Disabled";
  ...
  if(get_kb_item("Policy/linux/sysctl/conf/ERROR")){
    ...
    foreach file (files){
      ...
      grep_pattern = egrep(string:content, pattern:"(net.ipv4.conf.all.rp_filter|net.ipv4.conf.default.rp_filter)");
      if(grep_pattern){
