...
# $Id: win_network_selection_ui.nasl 10819 2018-08-07 14:11:07Z emoss $
#
# Check value for Do not display network selection UI
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312630");
  ...
  script_name('Microsoft Windows: Do not display network selection UI');
...
'Do not display network selection UI' on Windows hosts (at least Windows 8.1).

The setting allows to control whether anyone can interact with available networks UI on the logon
screen.");
  exit(0);
}
...
item = 'DontDisplayNetworkSelectionUI';
