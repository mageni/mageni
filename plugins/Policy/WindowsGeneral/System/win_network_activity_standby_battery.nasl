...
# $Id: win_network_activity_standby_battery.nasl 10819 2018-08-07 14:11:07Z emoss $
#
# Check value for Allow network connectivity during connected-standby
# (on battery)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312634");
  ...
  script_name('Microsoft Windows: Allow network connectivity during connected-standby (on battery)');
...
'Allow network connectivity during connected-standby (on battery)' on Windows hosts (at least
Windows 8.1).

The setting controls the network connectivity state in standby on modern standby-capable systems.");
  exit(0);
}
...
title = 'Allow network connectivity during connected-standby (on battery)';
