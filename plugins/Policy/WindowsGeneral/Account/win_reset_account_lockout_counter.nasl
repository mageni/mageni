...
# $Id: win_reset_account_lockout_counter.nasl 10649 2018-07-27 07:16:55Z emoss $
#
# Check value for Reset account lockout counter after (WMI)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312546");
  ...
  script_name('Microsoft Windows: Reset account lockout counter after');
...
'Reset account lockout counter after' on Windows hosts (at least Windows 7).

The policy setting determines the number of minutes that must elapse from the 
time a user fails to log on before the failed logon attempt counter is reset to 0.");
  exit(0);
}
