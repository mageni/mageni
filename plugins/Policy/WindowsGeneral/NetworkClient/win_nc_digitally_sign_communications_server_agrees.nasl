...
# $Id: win_nc_digitally_sign_communications_server_agrees.nasl 10740 2018-08-02 14:13:50Z emoss $
#
# Check value for Microsoft network client: Digitally sign communications (if server agrees)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312592");
  ...
  script_name('Microsoft Windows: Microsoft network client: Digitally sign communications (if server agrees)');
...
'Microsoft network client: Digitally sign communications (if server agrees)' on Windows
...
attacks. If enabled, the SMB client will communicate with an SMB server with SMB
packet signing, if possible");
  exit(0);
}
