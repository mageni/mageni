...
# $Id: win_share_files_profile.nasl 10989 2018-08-15 14:57:51Z emoss $
#
# Check value for Prevent users from sharing files within their profile
# (users listed in HKU)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312506");
...
'Prevent users from sharing files within their profile' on Windows hosts (at least
Windows 7).

The setting specifies whether users can share files within their profile. By
default users are allowed to share files within their profile to other users on
their network after an administrator opts in the computer. An administrator can
opt in the computer by using the sharing wizard to share a file within their
profile.");
  exit(0);
}
...
foreach uid (uids){
  if(uid =~ "S-1-5-21-"){
      ...
      key = uid + '\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer';
