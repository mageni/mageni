...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312499");
  ...
  script_name('Microsoft Windows: Turn off Help Experience Improvement Program');
...
'Turn off Help Experience Improvement Program' on Windows hosts (at least
Windows 7).

The setting specifies whether users can participate in the Help Experience
Improvement program. The Help Experience Improvement program collects
information about how customers use Windows Help so that Microsoft can improve it.");
  exit(0);
}
...
User Configuration/Administrative Templates/System/Internet Communication Management/Internet Communication Settings/' + title;
type = 'HKU';
item = 'NoImplicitFeedback';
