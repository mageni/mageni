...
cleanup_leftovers (int num_files)
{
  ...
  proctitle_set ("mageni-vscand: Cleaning leftover NVTs.");
...
plugins_reload_from_dir (void *folder)
{
      ...
      if (loaded_files % 50 == 0)
        {
          ...
          proctitle_set ("mageni-vscand: Reloaded %d of %d KB"
                         " (%d%% / ETA: %02d:%02d)",
      ...
      if (err_count == 20)
        {
          ...
          proctitle_set ("mageni-vscand: Error loading NVTs.");
  ...
  proctitle_set ("mageni-vscand: Reloaded all the KB.");
