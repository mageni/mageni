            ...
            GHashTable *names_table, int autoload, char *oid)
{
  ...
  if (!(category >= ACT_INIT && category <= ACT_END))
    {
      g_message ("The NVT with oid %s has no category assigned. This  "
                 "means that the structure store in Redis out dated. "
                 "Please, run a system update.",
                 oid);
      nvti_free (nvti);
      /* Return 0 in order to prevent the scan from returning interrupted at 1%*/
      return 0;
    }
