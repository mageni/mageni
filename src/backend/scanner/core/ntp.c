 ...
 * SPDX-FileComment: Mageni Transfer Protocol handling.
...
#include <glib.h>
#include "../../libraries/base/prefs.h" /* for prefs_set() */
...
ntp_read_prefs (int soc)
{
  ...
  if (buffer_size_count >= input_sz) /* finished = 1; */
    {
      g_warning ("%s: The received plugin list is longer than expected. ",
                  __func__);
    }

  g_warning ("%s: Received %d chars from the client as plugin list. ",
