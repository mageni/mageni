...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.811286");
  ...
  script_name("Microsoft SQL Server 2016 Information Disclosure Vulnerability (KB4019095)");
...
foreach item (registry_enum_keys(key:ms_sql_key))
{
  ...
  if("Microsoft SQL Server" >< sql_path){
    ...
    if(sysVer =~ "^13\.0"){
      if(version_in_range(version:sysVer, test_version:"13.0.2000.0", test_version2:"13.0.4445.0")){
        report = 'File checked:     ' + sql_path + "\microsoft.sqlserver.chainer.infrastructre.dll" + '\n' +
                 'File version:     ' + sysVer  + '\n' +
                 'Vulnerable range: 13.0.2000.0 - 13.0.4445.0\n' ;
