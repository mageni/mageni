...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150088");
  ...
  script_name("Linux: Read /etc/hosts.allow (KB)");
...
if(!get_kb_item("login/SSH/success")){
  set_kb_item(name:"Policy/linux/etc/hosts_allow/ERROR", value:TRUE);
  set_kb_item(name:"Policy/linux/etc/hosts_allow/stat/ERROR", value:TRUE);
  exit(0);
}
...
stat_cmd = "stat /etc/hosts.allow";
stat_ret = ssh_cmd_without_errors(socket:sock, cmd:stat_cmd);
if(!stat_ret){
  set_kb_item(name:"Policy/linux/etc/hosts_allow/stat/ERROR", value:TRUE);
  exit(0);
}
set_kb_item(name:"Policy/linux/etc/hosts_allow/stat", value:stat_ret);

cat_cmd = "cat /etc/hosts.allow";
cat_ret = ssh_cmd_without_errors(socket:sock, cmd:cat_cmd);
if(!cat_ret){
  set_kb_item(name:"Policy/linux/etc/hosts_allow/ERROR", value:TRUE);
  exit(0);
}
set_kb_item(name:"Policy/linux/etc/hosts_allow", value:cat_ret);
