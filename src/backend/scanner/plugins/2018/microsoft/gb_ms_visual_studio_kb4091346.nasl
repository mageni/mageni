...
if(description)
{
  ...
  script_xref(name:"URL", value:"https://support.microsoft.com/en-us/help/4091346");
...
if("x86" >< os_arch){
  key_list = make_list("SOFTWARE\Microsoft\VisualStudio\10.0");
}

else if("x64" >< os_arch){
 key_list = make_list("SOFTWARE\Microsoft\VisualStudio\10.0", "SOFTWARE\Wow6432Node\Microsoft\VisualStudio\10.0");
}
...
foreach key (key_list)
{
  ...
  if(dllVer && dllVer =~ "^10\.0" && version_is_less(version:dllVer, test_version:"10.0.40219.478"))
  {
    report = report_fixed_ver(file_checked: installPath + "mspdbsrv.exe",
                              file_version:dllVer, vulnerable_range:"10.0 - 10.0.40219.477");
