...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2024.1044");
  ...
  script_name("Huawei EulerOS: Security Advisory for mdadm (EulerOS-SA-2024-1044)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2024-1044");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2024-1044");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'mdadm' package(s) announced via the EulerOS-SA-2024-1044 advisory.");
...
Buffer overflow in some Intel(R) SSD Tools software before version mdadm-4.2-rc2 may allow a privileged user to potentially enable escalation of privilege via local access.(CVE-2023-28736)");
  ...
  script_tag(name:"affected", value:"'mdadm' package(s) on Huawei EulerOS Virtualization release 2.9.1.");
...
if(release == "EULEROSVIRT-2.9.1") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"mdadm", rpm:"mdadm~4.1~rc2.0.9.h3.r3.eulerosv2r9", rls:"EULEROSVIRT-2.9.1"))) {
    report += res;
  }
