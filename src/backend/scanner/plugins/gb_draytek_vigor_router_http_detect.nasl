...
if(description)
{
  ...
  script_name("DrayTek Vigor Router Detection (HTTP)");

  script_tag(name:"summary", value:"Detection of DrayTek Vigor Router.

  This script performs HTTP based detection of DrayTek Vigor Router.");
...
if (("<title>Vigor " >< res && "isomorphicDir" >< res && res =~ "Server\s*:\s*DWS") ||
  ...
  set_kb_item(name: "draytek/vigor/router/http/port", value: port);

  mod = eregmatch(pattern: "<title>Vigor ([0-9A-Z]+)", string: res);
  if (!isnull(mod[1]) && "Vigor Login Page" >!< res) {
    model = mod[1];
    set_kb_item(name: "draytek/vigor/router/http/" + port + "/concluded", value: mod[0]);
  }
  ...
  if (isnull(mod[1])) {
    foreach url( make_list( "/images/login.png", "/images/login1.png" ) ) {
      ...
      if(res) {
        ...
        if (fp) {
          model = fp;
          set_kb_item(name: "draytek/vigor/router/http/" + port + "/concludedUrl", value: report_vuln_url(port: port, url: url, url_only: TRUE));
          break;
        }
      }
    }
  ...
  set_kb_item(name: "draytek/vigor/router/http/" + port + "/model", value: model);
  set_kb_item(name: "draytek/vigor/router/http/" + port + "/version", value: version);
}
