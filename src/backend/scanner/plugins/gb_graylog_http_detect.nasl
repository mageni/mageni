...
if (description)
{
  ...
  script_name("Graylog Detection (HTTP)");

  script_tag(name:"summary", value:"HTTP based detection of Graylog.");
...
if( "<title>Graylog Web Interface</title>" >!< res || "X-Graylog-Node-ID" >!< res )
  exit( 0 );

version = "unknown";

set_kb_item( name:"graylog/detected", value:TRUE );
set_kb_item( name:"graylog/http/detected", value:TRUE );
set_kb_item( name:"graylog/http/port", value:port );
