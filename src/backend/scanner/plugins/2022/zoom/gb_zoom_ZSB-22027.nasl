...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104424");
  ...
  script_cve_id("CVE-2022-28766");

  script_tag(name:"qod_type", value:"executable_version");

  script_tag(name:"solution_type", value:"VendorFix");

  script_name("Zoom Client < 5.12.6 DLL Injection Vulnerability (ZSB-22027)");
  ...
  script_mandatory_keys("zoom/client/win/detected", "SMB/Windows/Arch");

  script_tag(name:"summary", value:"The Zoom Client is prone to a DLL injection vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"Windows 32-bit versions of the Zoom Client are susceptible to a
  DLL injection vulnerability.");

  script_tag(name:"impact", value:"A local low-privileged user could exploit this vulnerability to
  run arbitrary code in the context of the Zoom client.");

  script_tag(name:"affected", value:"Zoom Client prior to version 5.12.6 on Windows (32-bit).");
