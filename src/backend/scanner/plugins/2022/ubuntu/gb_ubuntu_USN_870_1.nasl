...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.12.2009.870.1");
  ...
  script_name("Ubuntu: Security Advisory (USN-870-1)");
  ...
  script_xref(name:"Advisory-ID", value:"USN-870-1");
  script_xref(name:"URL", value:"https://ubuntu.com/security/notices/USN-870-1");

  script_tag(name:"summary", value:"The remote host is missing an update for the 'pygresql' package(s) announced via the USN-870-1 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Steffen Joeris discovered that PyGreSQL 3.8 did not use PostgreSQL's safe
...
should be adjusted to use:
 ...
 escaped = connection.escape_string(untrusted_input)");
  ...
  script_tag(name:"affected", value:"'pygresql' package(s) on Ubuntu 8.04, Ubuntu 8.10.");
...
if(release == "UBUNTU8.04 LTS") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"python-pygresql", ver:"1:3.8.1-2ubuntu0.1", rls:"UBUNTU8.04 LTS"))) {
    report += res;
  }
...
if(release == "UBUNTU8.10") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"python-pygresql", ver:"1:3.8.1-3ubuntu0.1", rls:"UBUNTU8.10"))) {
    report += res;
  }
