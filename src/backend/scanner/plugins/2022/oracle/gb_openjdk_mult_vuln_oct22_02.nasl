...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104365");
  ...
  script_name("Oracle OpenJDK Multiple Vulnerabilities (Oct 2022 - 02)");
...
if (vers =~ "^1\.7" && version_is_less(version: vers, test_version: "1.7.0.361")) {
  report = report_fixed_ver(installed_version: vers, fixed_version: "1.7.0.361", install_path: path);
  security_message(port: port, data: report);
  exit(0);
}

if (vers =~ "^1\.8" && version_is_less(version: vers, test_version: "1.8.0.352")) {
  report = report_fixed_ver(installed_version: vers, fixed_version: "1.8.0.352", install_path: path);
  security_message(port: port, data: report);
  exit(0);
}
