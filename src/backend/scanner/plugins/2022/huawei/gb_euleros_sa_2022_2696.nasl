...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2022.2696");
  ...
  script_name("Huawei EulerOS: Security Advisory for rsync (EulerOS-SA-2022-2696)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2022-2696");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2022-2696");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'rsync' package(s) announced via the EulerOS-SA-2022-2696 advisory.");
...
An issue was discovered in rsync before 3.2.5 that allows malicious remote servers to write arbitrary files inside the directories of connecting peers. The server chooses which files/directories are sent to the client. However, the rsync client performs insufficient validation of file names. A malicious rsync server (or Man-in-The-Middle attacker) can overwrite arbitrary files in the rsync client target directory and subdirectories (for example, overwrite the .ssh/authorized_keys file).(CVE-2022-29154)");
  ...
  script_tag(name:"affected", value:"'rsync' package(s) on Huawei EulerOS V2.0SP10(x86_64).");
...
if(release == "EULEROS-2.0SP10-x86_64") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"rsync", rpm:"rsync~3.1.3~6.h4.eulerosv2r10", rls:"EULEROS-2.0SP10-x86_64"))) {
    report += res;
  }
