...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2022.2033");
  ...
  script_name("Huawei EulerOS: Security Advisory for polkit (EulerOS-SA-2022-2033)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2022-2033");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2022-2033");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'polkit' package(s) announced via the EulerOS-SA-2022-2033 advisory.");
...
There is a flaw in polkit which can allow an unprivileged user to cause polkit to crash, due to process file descriptor exhaustion. The highest threat from this vulnerability is to availability. NOTE: Polkit process outage duration is tied to the failing process being reaped and a new one being spawned(CVE-2021-4115)");
  ...
  script_tag(name:"affected", value:"'polkit' package(s) on Huawei EulerOS Virtualization release 2.10.0.");
...
if(release == "EULEROSVIRT-2.10.0") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"polkit", rpm:"polkit~0.116~6.h5.eulerosv2r10", rls:"EULEROSVIRT-2.10.0"))) {
    report += res;
  }

  if(!isnull(res = isrpmvuln(pkg:"polkit-libs", rpm:"polkit-libs~0.116~6.h5.eulerosv2r10", rls:"EULEROSVIRT-2.10.0"))) {
    report += res;
  }
