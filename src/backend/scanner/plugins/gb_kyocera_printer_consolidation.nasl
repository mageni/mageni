...
if (description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.147517");
  script_version("2022-01-31T06:14:44+0000");
  script_tag(name:"last_modification", value:"2022-02-01 11:05:08 +0000 (Tue, 01 Feb 2022)");
  script_tag(name:"creation_date", value:"2022-01-26 07:59:36 +0000 (Wed, 26 Jan 2022)");
  script_tag(name:"cvss_base", value:"0.0");
  script_tag(name:"cvss_base_vector", value:"AV:N/AC:L/Au:N/C:N/I:N/A:N");

  script_tag(name:"qod_type", value:"remote_banner");

  script_name("Kyocera Printer Detection Consolidation");

  script_tag(name:"summary", value:"Consolidation of Kyocera Printer device detections.");

  script_category(ACT_GATHER_INFO);

  script_copyright("Copyright (C) 2022 Greenbone Networks GmbH");
  script_family("Product detection");
  script_dependencies("gb_kyocera_printer_http_detect.nasl",
                      "gb_kyocera_printer_snmp_detect.nasl",
                      "gb_kyocera_printer_ftp_detect.nasl",
                      "gb_kyocera_printer_pjl_detect.nasl",
                      "global_settings.nasl");
  script_mandatory_keys("kyocera/printer/detected");

  script_xref(name:"URL", value:"https://www.kyoceradocumentsolutions.com/");
...
if (!get_kb_item("kyocera/printer/detected"))
  exit(0);
...
foreach source (make_list("http", "snmp", "ftp", "hp-pjl")) {
  model_list = get_kb_list("kyocera/printer/" + source + "/*/model");
  ...
  fw_version_list = get_kb_list("kyocera/printer/" + source + "/*/fw_version");
...
if (detected_model != "unknown") {
  ...
  if (detected_fw_version != "unknown")
    os_cpe = build_cpe(value: tolower(detected_fw_version), exp: "^([0-9a-z._]+)",
                       base: "cpe:/o:kyocera:" + cpe_model + "_firmware:");
  else
    os_cpe = "cpe:/o:kyocera:" + cpe_model + "_firmware";

  hw_cpe = "cpe:/h:kyocera:" + cpe_model;
} else {
  ...
  if (detected_fw_version != "unknown")
    os_cpe = build_cpe(value: tolower(detected_fw_version), exp: "^([0-9a-z._]+)",
                       base: "cpe:/o:kyocera:printer_firmware:");
  else
    os_cpe = "cpe:/o:kyocera:printer_firmware";

  hw_cpe = "cpe:/h:kyocera:printer";
}

if (http_ports = get_kb_list("kyocera/printer/http/port")) {
  foreach port (http_ports) {
    extra += 'HTTP(s) on port ' + port + '/tcp\n';

    modConcluded = get_kb_item("kyocera/printer/http/" + port + "/modConcluded");
    modConcludedUrl = get_kb_item("kyocera/printer/http/" + port + "/modConcludedUrl");
    versConcluded = get_kb_item("kyocera/printer/http/" + port + "/versConcluded");
    versConcludedUrl = get_kb_item("kyocera/printer/http/" + port + "/versConcludedUrl");
    ...
    generalConcluded = get_kb_item("kyocera/printer/http/" + port + "/generalConcluded");
    if (generalConcluded) {
...
if (snmp_ports = get_kb_list("kyocera/printer/snmp/port")) {
  foreach port (snmp_ports) {
    extra += 'SNMP on port ' + port + '/udp\n';

    concludedMod = get_kb_item("kyocera/printer/snmp/" + port + "/concludedMod");
    concludedModOID = get_kb_item("kyocera/printer/snmp/" + port + "/concludedModOID");
    if (concludedMod && concludedModOID)
      extra += '  Model concluded from "' + concludedMod + '" via OID: ' + concludedModOID + '\n';

    concludedFwOID = get_kb_item("kyocera/printer/snmp/" + port + "/concludedFwOID");
    if (concludedFwOID)
...
if (pjl_ports = get_kb_list("kyocera/printer/hp-pjl/port")) {
  foreach port (pjl_ports) {
    extra += 'PJL on port ' + port + '/tcp\n';

    concluded = get_kb_item("kyocera/printer/hp-pjl/" + port + "/concluded");
    if (concluded)
...
if (ftp_ports = get_kb_list("kyocera/printer/ftp/port")) {
  foreach port (ftp_ports) {
    extra += 'FTP on port ' + port + '/tcp\n';

    concluded = get_kb_item("kyocera/printer/ftp/"+ port + "/concluded");
    if (concluded)
...
os_register_and_report(os: os_name, cpe: os_cpe, runs_key: "unixoide",
                       desc: "Kyocera Printer Detection Consolidation");
