...
if(description)
{
  ...
  script_name("PHPGenealogie Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of PHPGenealogie.");
...
foreach dir(make_list_unique("/geneald", "/genealogie_sql", "/genealogie", http_cgi_dirs(port:port))) {

  install = dir;
  if(dir == "/")
    dir = "";

  res = http_get_cache(item:dir + "/Index2.php", port:port);
  if(!res || res !~ "^HTTP/1\.[01] 200")
    continue;

  if('">php.genealogie' >< res) {
    ...
    set_kb_item(name:"phpgenealogie/http/detected", value:TRUE);
