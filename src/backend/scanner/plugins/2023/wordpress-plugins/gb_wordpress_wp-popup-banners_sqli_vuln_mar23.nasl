...
if (description)
{
  ...
  script_cve_id("CVE-2023-28661");

  script_tag(name:"qod_type", value:"remote_banner");

  script_tag(name:"solution_type", value:"WillNotFix");

  script_name("WordPress WP Popup Banners Plugin <= 1.2.5 SQLi Vulnerability");
  ...
  script_tag(name:"summary", value:"The WordPress plugin 'WP Popup Banners' is prone to an SQL
  injection (SQLi) vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The plugin does not properly sanitise and escape the value
  parameter before using it in a SQL statement via the get_popup_data AJAX action, leading to a SQL
  injection exploitable by any authenticated users, such as subscriber.");
