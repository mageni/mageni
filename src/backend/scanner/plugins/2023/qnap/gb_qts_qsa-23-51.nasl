...
if(description)
{
  ...
  script_name("QNAP QTS SSRF Vulnerability (QSA-23-51)");
  ...
  script_tag(name:"summary", value:"QNAP QTS is prone to a server-side request forgery (SSRF)
  vulnerability.");
  ...
  script_tag(name:"solution", value:"Update to version 5.0.1.2514 build 20230906, 5.1.1.2491 build
  20230815 or later.");
...
if (version =~ "^5\.0") {
  if (version_is_less(version: version, test_version:"5.0.1.2514")) {
    report = report_fixed_ver(installed_version: version, installed_build: build, fixed_version: "5.0.1.2514", fixed_build: "20230906");
...
if (version =~ "^5\.1") {
  if (version_is_less(version: version, test_version:"5.1.1.2491")) {
    report = report_fixed_ver(installed_version: version, installed_build: build, fixed_version: "5.1.1.2491", fixed_build: "20230815");
    security_message(port: 0, data: report);
    exit(0);
  }

  if (version_is_equal(version: version, test_version: "5.1.1.2491") &&
     (!build || version_is_less(version: build, test_version: "20230815"))) {
    report = report_fixed_ver(installed_version: version, installed_build: build, fixed_version: "5.1.1.2491", fixed_build: "20230815");
    security_message(port: 0, data: report);
    exit(0);
  }
