...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2023.3193");
  ...
  script_name("Huawei EulerOS: Security Advisory for qemu-micro (EulerOS-SA-2023-3193)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2023-3193");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2023-3193");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'qemu-micro' package(s) announced via the EulerOS-SA-2023-3193 advisory.");
...
A flaw was found in the QEMU virtual crypto device while handling data encryption/decryption requests in virtio_crypto_handle_sym_req. There is no check for the value of `src_len` and `dst_len` in virtio_crypto_sym_op_helper, potentially leading to a heap buffer overflow when the two values differ.(CVE-2023-3180)");
  ...
  script_tag(name:"affected", value:"'qemu-micro' package(s) on Huawei EulerOS V2.0SP10.");
...
if(release == "EULEROS-2.0SP10") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"qemu-img", rpm:"qemu-img~4.1.0~35.h7.eulerosv2r10", rls:"EULEROS-2.0SP10"))) {
    report += res;
  }
