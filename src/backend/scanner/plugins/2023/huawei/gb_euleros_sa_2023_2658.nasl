...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2023.2658");
  ...
  script_name("Huawei EulerOS: Security Advisory for ntp (EulerOS-SA-2023-2658)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2023-2658");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2023-2658");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'ntp' package(s) announced via the EulerOS-SA-2023-2658 advisory.");
...
mstolfp in libntp/mstolfp.c in NTP 4.2.8p15 has an out-of-bounds write when copying the trailing number. An adversary may be able to attack a client ntpq process, but cannot attack ntpd.(CVE-2023-26553)");
  ...
  script_tag(name:"affected", value:"'ntp' package(s) on Huawei EulerOS V2.0SP11.");
...
if(release == "EULEROS-2.0SP11") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"ntp", rpm:"ntp~4.2.8p15~5.h5.eulerosv2r11", rls:"EULEROS-2.0SP11"))) {
    report += res;
  }

  if(!isnull(res = isrpmvuln(pkg:"ntp-help", rpm:"ntp-help~4.2.8p15~5.h5.eulerosv2r11", rls:"EULEROS-2.0SP11"))) {
    report += res;
  }
