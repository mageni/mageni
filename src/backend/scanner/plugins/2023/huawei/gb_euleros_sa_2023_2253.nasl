...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2023.2253");
  ...
  script_name("Huawei EulerOS: Security Advisory for libXpm (EulerOS-SA-2023-2253)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2023-2253");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2023-2253");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'libXpm' package(s) announced via the EulerOS-SA-2023-2253 advisory.");
...
A flaw was found in libXpm. This issue occurs when parsing a file with a comment not closed, the end-of-file condition will not be detected, leading to an infinite loop and resulting in a Denial of Service in the application linked to the library.(CVE-2022-46285)");
  ...
  script_tag(name:"affected", value:"'libXpm' package(s) on Huawei EulerOS Virtualization for ARM 64 3.0.6.0.");
...
if(release == "EULEROSVIRTARM64-3.0.6.0") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"libXpm", rpm:"libXpm~3.5.12~8.eulerosv2r8", rls:"EULEROSVIRTARM64-3.0.6.0"))) {
    report += res;
  }

  if(!isnull(res = isrpmvuln(pkg:"libXpm-devel", rpm:"libXpm-devel~3.5.12~8.eulerosv2r8", rls:"EULEROSVIRTARM64-3.0.6.0"))) {
    report += res;
  }
