...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2023.3274");
  ...
  script_name("Huawei EulerOS: Security Advisory for iSulad (EulerOS-SA-2023-3274)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2023-3274");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2023-3274");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'iSulad' package(s) announced via the EulerOS-SA-2023-3274 advisory.");
...
When the isula cp command is used to copy files from a container to a host machine and the container is controlled by an attacker, the attacker can escape the container.(CVE-2021-33638)");
  ...
  script_tag(name:"affected", value:"'iSulad' package(s) on Huawei EulerOS V2.0SP11(x86_64).");
...
if(release == "EULEROS-2.0SP11-x86_64") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"iSulad", rpm:"iSulad~2.0.11~6.h98.eulerosv2r11", rls:"EULEROS-2.0SP11-x86_64"))) {
    report += res;
  }
