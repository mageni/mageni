...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.832622");
  ...
  script_name("Microsoft .NET Framework Multiple Vulnerabilities (KB5031989)");

  script_tag(name:"summary", value:"This host is missing an important security
  update according to Microsoft KB5031989");
  ...
  script_xref(name:"URL", value:"https://support.microsoft.com/en-us/help/5031989");
    ...
    foreach item (registry_enum_keys(key:key))
    {
      ...
      if(NetPath && "\Microsoft.NET\Framework" >< NetPath)
      {
        foreach item (registry_enum_keys(key:key))
        {
          ...
          if(dllVer)
          {
            if(version_in_range(version:dllVer, test_version:"4.8", test_version2:"4.8.4681.0"))
            {
              VULN = TRUE ;
  ...
  if((!vulnerable_range) && "ASP.NET" >< key)
  {
    foreach item (registry_enum_keys(key:key))
    {
      ...
      if(dotPath && "\Microsoft.NET\Framework" >< dotPath)
      {
        ...
        if(dllVer)
        {
          if(version_in_range(version:dllVer, test_version:"4.8", test_version2:"4.8.4681"))
          {
            VULN = TRUE ;
