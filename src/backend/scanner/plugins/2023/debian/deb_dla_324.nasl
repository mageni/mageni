...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.2.2015.324");
  script_cve_id("CVE-2012-3509", "CVE-2014-9939");
  ...
  script_name("Debian: Security Advisory (DLA-324)");
  ...
  script_xref(name:"Advisory-ID", value:"DLA-324");
  script_xref(name:"URL", value:"https://www.debian.org/lts/security/2015/dla-324");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'binutils' package(s) announced via the DLA-324 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"This update fixes several issues as described below.
...
CVE-2012-3509, #688951 Sang Kil Cha discovered that a buffer size calculation in libiberty, part of binutils, may result in integer overflow and then a heap buffer overflow. If libiberty or the commands in binutils are used to read untrusted binaries, this would allow remote attackers to cause a denial of service (crash) or possibly privilege escalation.
...
For the oldstable distribution (wheezy) and the stable distribution (jessie), PR ld/12613 and CVE-2012-3509 were fixed before release, and PR binutils/18750 will be fixed in a later update.");
  ...
  script_tag(name:"affected", value:"'binutils' package(s) on Debian 6.");
...
if(release == "DEB6") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"binutils-dev", ver:"2.20.1-16+deb6u2", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"binutils-doc", ver:"2.20.1-16+deb6u2", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"binutils-gold", ver:"2.20.1-16+deb6u2", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"binutils-multiarch", ver:"2.20.1-16+deb6u2", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"binutils-source", ver:"2.20.1-16+deb6u2", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"binutils", ver:"2.20.1-16+deb6u2", rls:"DEB6"))) {
    report += res;
  }
