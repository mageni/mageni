...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.1.2010.2136");
  ...
  script_name("Debian: Security Advisory (DSA-2136)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-2136");
  script_xref(name:"URL", value:"https://www.debian.org/security/2010/dsa-2136");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'tor' package(s) announced via the DSA-2136 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Willem Pinckaers discovered that Tor, a tool to enable online anonymity, does not correctly handle all data read from the network. By supplying specially crafted packets a remote attacker can cause Tor to overflow its heap, crashing the process. Arbitrary code execution has not been confirmed but there is a potential risk.
...
if(release == "DEB5") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"tor-dbg", ver:"0.2.1.26-1~lenny+4", rls:"DEB5"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"tor-geoipdb", ver:"0.2.1.26-1~lenny+4", rls:"DEB5"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"tor", ver:"0.2.1.26-1~lenny+4", rls:"DEB5"))) {
    report += res;
  }
