...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.2.2015.188");
  ...
  script_name("Debian: Security Advisory (DLA-188)");
  ...
  script_xref(name:"Advisory-ID", value:"DLA-188");
  script_xref(name:"URL", value:"https://www.debian.org/lts/security/2015/dla-188");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'arj' package(s) announced via the DLA-188 advisory.");
...
Jakub Wilk and Guillem Jover discovered a buffer overflow vulnerability in arj. A remote attacker could use this flaw to cause an application crash or, possibly, execute arbitrary code with the privileges of the user running arj.");
  ...
  script_tag(name:"affected", value:"'arj' package(s) on Debian 6.");
...
if(release == "DEB6") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"arj", ver:"3.10.22-9+deb6u1", rls:"DEB6"))) {
    report += res;
  }
