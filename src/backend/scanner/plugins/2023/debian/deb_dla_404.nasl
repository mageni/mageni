...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.2.2016.404");
  script_cve_id("CVE-2016-0742");
  ...
  script_name("Debian: Security Advisory (DLA-404)");
  ...
  script_xref(name:"Advisory-ID", value:"DLA-404");
  script_xref(name:"URL", value:"https://www.debian.org/lts/security/2016/dla-404");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'nginx' package(s) announced via the DLA-404 advisory.");
...
if(release == "DEB6") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"nginx-dbg", ver:"0.7.67-3+squeeze4+deb6u1", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"nginx", ver:"0.7.67-3+squeeze4+deb6u1", rls:"DEB6"))) {
    report += res;
  }
