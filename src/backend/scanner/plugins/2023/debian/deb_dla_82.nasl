...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.2.2014.82");
  ...
  script_name("Debian: Security Advisory (DLA-82)");
  ...
  script_xref(name:"Advisory-ID", value:"DLA-82");
  script_xref(name:"URL", value:"https://www.debian.org/lts/security/2014/dla-82");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'wget' package(s) announced via the DLA-82 advisory.");
...
For Debian 6 Squeeze, these issues have been fixed in wget version 1.12-2.1+deb6u1");
  ...
  script_tag(name:"affected", value:"'wget' package(s) on Debian 6.");
...
if(release == "DEB6") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"wget", ver:"1.12-2.1+deb6u1", rls:"DEB6"))) {
    report += res;
  }
