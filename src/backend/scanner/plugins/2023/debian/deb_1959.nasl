...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.1.2009.1959");
  script_cve_id("CVE-2009-4261");
  ...
  script_name("Debian: Security Advisory (DSA-1959)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-1959");
  script_xref(name:"URL", value:"https://www.debian.org/security/2009/dsa-1959");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'ganeti' package(s) announced via the DSA-1959 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"It was discovered that ganeti, a virtual server cluster manager, does not validate the path of scripts passed as arguments to certain commands, which allows local or remote users (via the web interface in versions 2.x) to execute arbitrary commands on a host acting as a cluster master.
...
We recommend that you upgrade your ganeti packages.");
  ...
  script_tag(name:"affected", value:"'ganeti' package(s) on Debian 5.");
...
if(release == "DEB5") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"ganeti", ver:"1.2.6-3+lenny2", rls:"DEB5"))) {
    report += res;
  }
