...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.2.2014.68");
  ...
  script_name("Debian: Security Advisory (DLA-68)");
  ...
  script_xref(name:"Advisory-ID", value:"DLA-68");
  script_xref(name:"URL", value:"https://www.debian.org/lts/security/2014/dla-68");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'fex' package(s) announced via the DLA-68 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"[CVE-2014-3875] When inserting encoded newline characters into a request to rup, additional HTTP headers can be injected into the reply, as well as new HTML code on the top of the website.
...
if(release == "DEB6") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"fex-utils", ver:"20100208+debian1-1+squeeze4", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"fex", ver:"20100208+debian1-1+squeeze4", rls:"DEB6"))) {
    report += res;
  }
