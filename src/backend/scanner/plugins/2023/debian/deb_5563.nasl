...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.1.2023.5563");
  ...
  script_name("Debian: Security Advisory (DSA-5563-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-5563-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2023/DSA-5563-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-5563");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/intel-microcode");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'intel-microcode' package(s) announced via the DSA-5563-1 advisory.");
...
For the detailed security status of intel-microcode please refer to its security tracker page at: [link moved to references]");
  ...
  script_tag(name:"affected", value:"'intel-microcode' package(s) on Debian 11, Debian 12.");
...
if(release == "DEB11") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"intel-microcode", ver:"3.20231114.1~deb11u1", rls:"DEB11"))) {
    report += res;
  }
...
if(release == "DEB12") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"intel-microcode", ver:"3.20231114.1~deb12u1", rls:"DEB12"))) {
    report += res;
  }
