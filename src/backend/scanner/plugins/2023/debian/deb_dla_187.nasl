...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.1.2.2015.187");
  ...
  script_name("Debian: Security Advisory (DLA-187)");
  ...
  script_xref(name:"Advisory-ID", value:"DLA-187");
  script_xref(name:"URL", value:"https://www.debian.org/lts/security/2015/dla-187");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'tor' package(s) announced via the DLA-187 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Several hidden service related denial-of-service issues have been discovered in Tor, a connection-based low-latency anonymous communication system.
...
if(release == "DEB6") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"tor-dbg", ver:"0.2.4.27-1~deb6u1", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"tor-geoipdb", ver:"0.2.4.27-1~deb6u1", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"tor", ver:"0.2.4.27-1~deb6u1", rls:"DEB6"))) {
    report += res;
  }
