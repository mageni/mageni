...
# $Id: gb_nmap_nfs_statfs_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  ...
  script_add_preference(name:"nfs-statfs.human", value:"", type:"entry");
...
if (phase == 1) {
    ...
    pref = script_get_preference("nfs-statfs.human");
    if (!isnull(pref) && pref != "") {
        argv["nfs-statfs.human"] = string('"', pref, '"');
    }
    ...
    nmap_nse_register(script:"nfs-statfs", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"nfs-statfs");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (nfs-statfs.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
