...
# $Id: gb_nmap_imap_capabilities_net.nasl 11527 2018-09-21 15:59:27Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104055");
  ...
  script_name("Nmap NSE net: imap-capabilities");
...
if (phase == 1) {
  argv = make_array();
  nmap_nse_register(script:"imap-capabilities", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"imap-capabilities");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (imap-capabilities.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
