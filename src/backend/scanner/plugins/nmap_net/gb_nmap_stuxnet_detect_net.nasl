...
# $Id: gb_nmap_stuxnet_detect_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104104");
  ...
  script_name("Nmap NSE net: stuxnet-detect");
  ...
  script_add_preference(name:"stuxnet-detect.save", value:"", type:"entry");
  ...
  script_xref(name:"URL", value:"http://en.wikipedia.org/wiki/Stuxnet");

  script_tag(name:"summary", value:"Detects whether a host is infected with the Stuxnet worm.

An executable version of the Stuxnet infection will be downloaded if a format for the filename is
...
stuxnet-detect.save:  Path to save Stuxnet executable under, with
'%h/code> replaced by the host's IP address, and '%v'
...
if (phase == 1) {
    ...
    pref = script_get_preference("stuxnet-detect.save");
    if (!isnull(pref) && pref != "") {
        argv["stuxnet-detect.save"] = string('"', pref, '"');
    }
    ...
    nmap_nse_register(script:"stuxnet-detect", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"stuxnet-detect");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (stuxnet-detect.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
