...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.118198");
  ...
  script_name("Python 2.x < 2.7.18, 3.x < 3.5.10, 3.6.x < 3.6.11, 3.7.x < 3.7.8, 3.8.x < 3.8.3 CRLF Injection Vulnerability (bpo-38576) - Mac OS X");
  ...
  script_tag(name:"summary", value:"Python is prone to a CRLF injection vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"http.client allows to pass control characters like CRLF newlines
  which can be abused to inject HTTP headers.");

  script_tag(name:"affected", value:"Python 2.x prior to version 2.7.18, versions 3.x prior to 3.5.10,
  3.6.x prior to 3.6.11, 3.7.x prior to 3.7.8 and 3.8.x prior to 3.8.3.");

  script_tag(name:"solution", value:"The vendor has released updates. Please see the references for
  more information.");

  script_xref(name:"URL", value:"https://python-security.readthedocs.io/vuln/urlopen-host-http-header-injection.html");
  script_xref(name:"Advisory-ID", value:"bpo-38576");
...
if( version_in_range( version:version, test_version:"2.0", test_version2:"2.7.17" ) ) {
  report = report_fixed_ver( installed_version:version, fixed_version:"2.7.18", install_path:location );
  security_message( port:port, data:report );
  exit( 0 );
}

if( version_in_range( version:version, test_version:"3.0", test_version2:"3.5.9" ) ) {
