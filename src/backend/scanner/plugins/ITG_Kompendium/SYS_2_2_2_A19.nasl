...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150005");
...
"gsf/Policy/WindowsGeneral/NetworkAccess/win_na_allow_storage_password_credentials.nasl");
...
Die Kern-Anforderung 'A19: Verwendung der Anmeldeinformationsverwaltung' beschreibt, dass die Verwendung
des lokalen Passwortspeichers ('Tresor') in einer Richtlinie festegelegt sein sollte.");
...
title = "Verwendung der Anmeldeinformationsverwaltung";
...
oid_list = make_list("1.3.6.1.4.1.25623.1.0.109220");

if (host_runs("windows_8.1") != "yes"){
  result = itg_result_wrong_target();
  desc = itg_desc_wrong_target();
  itg_set_kb_entries(result:result, desc:desc, title:title, id:"SYS.2.2.2.A19");
  exit(0);
}
