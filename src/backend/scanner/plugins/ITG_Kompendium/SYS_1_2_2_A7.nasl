...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150015");
...
Die Standard-Anforderung 'A7: Sicherheitspruefung von Windows Server 2012' beschreibt, dass die
Sicherheitskonfiguration des Hosts regelmaessig uebereprueft und dokumentiert werden sollte.");
...
title = "Sicherheitspruefung von Windows Server 2012";

if (host_runs("windows_server_2012") != "yes"){
  result = itg_result_wrong_target();
  desc = itg_desc_wrong_target();
  itg_set_kb_entries(result:result, desc:desc, title:title, id:"SYS.1.2.2.A7");
  exit(0);
}
