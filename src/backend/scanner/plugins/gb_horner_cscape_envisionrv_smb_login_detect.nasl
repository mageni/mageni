...
if(description)
{
  ...
  script_name("Horner APG Cscape EnvisionRV Detection (Windows SMB Login)");
  ...
  script_tag(name:"summary", value:"SMB login-based detection of Horner APG Cscape EnvisionRV.");
...
foreach key( key_list ) {
  foreach item( registry_enum_keys( key:key ) ) {
    ...
    if( ! app_name || app_name !~ "Cscape EnvisionRV" )
      continue;
    ...
    set_kb_item( name:"hornerautomation/cscape_envisionrv/smb-login/detected", value:TRUE );

    register_and_report_cpe( app:"Horner APG " + app_name, ver:version, concluded:concluded,
