...
solution = "sysctl -w net.ipv4.icmp_echo_ignore_broadcasts = [0,1]";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux/sysctl/ssh/ERROR")){
  ...
  if(!net_ipv4_icmp_echo_ignore_broadcasts){
    value = "Error";
    compliant = "incomplete";
  ...
  if(get_kb_item("Policy/linux/sysctl/conf/ERROR")){
    ...
    foreach file (files){
      ...
      grep_pattern = egrep(string:content, pattern:"net.ipv4.icmp_echo_ignore_broadcasts");
      if(grep_pattern){
