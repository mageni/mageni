...
cmd = "sysctl net.ipv4.conf.all.accept_redirects, sysctl net.ipv4.conf.default.accept_redirects";
title = "Accept ICMP Redirects";
solution = "sysctl -w net.ipv4.conf.all.accept_redirects = [0,1], sysctl -w sysctl net.ipv4.conf.default.accept_redirects = [0,1]";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux/sysctl/ssh/ERROR")){
  ...
  if(!net_ipv4_conf_all_accept_redirects || !net_ipv4_conf_default_accept_redirects){
    value = "Error";
    compliant = "incomplete";
    comment = "Could not find setting with sysctl";
  }else{
    if(net_ipv4_conf_all_accept_redirects == "0" && net_ipv4_conf_default_accept_redirects == "0")
      value = "Disabled";
    else
      value = "Enabled";
  ...
  if(get_kb_item("Policy/linux/sysctl/conf/ERROR")){
    ...
    foreach file (files){
      ...
      grep_pattern = egrep(string:content, pattern:"(net.ipv4.conf.all.accept_redirects|net.ipv4.conf.default.accept_redirects)");
      if(grep_pattern){
