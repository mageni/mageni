...
cmd = "sysctl net.ipv4.conf.all.accept_source_route, sysctl net.ipv4.conf.default.accept_source_route";
title = "Accept source routed packets";
solution = "sysctl -w net.ipv4.conf.all.accept_source_route = [0,1], sysctl -w net.ipv4.conf.default.accept_source_route = [0,1]";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux/sysctl/ssh/ERROR")){
  ...
  if(!net_ipv4_conf_all_accept_source_route || !net_ipv4_conf_default_accept_source_route){
    value = "Error";
    compliant = "incomplete";
    comment = "Could not find setting with sysctl";
  }else{
    if(net_ipv4_conf_all_accept_source_route == "0" && net_ipv4_conf_default_accept_source_route == "0")
      value = "Disabled";
    else
      value = "Enabled";
  ...
  if(get_kb_item("Policy/linux/sysctl/conf/ERROR")){
    ...
    foreach file (files){
      ...
      grep_pattern = egrep(string:content, pattern:"(net.ipv4.conf.all.accept_source_route|net.ipv4.conf.default.accept_source_route)");
      if(grep_pattern){
