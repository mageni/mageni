...
if(description)
{
  ...
  script_tag(name:"summary", value:"The udf file system is an industry-standard format for storing
information on DVDs and newer optical disc formats.

Linux kernel modules which implement filesystems that are not needed by the local system should be
disabled.

Note: This script looks for entry 'install udf /bin/true' in files in /etc/modprobe.d/*.conf and
...
cmd = "lsmod; grep -r 'install udf /bin/true' /etc/modprobe.d/*";
title = "Mounting of udf filesystems";
solution = "Add or remove 'install udf /bin/true' in config file (/etc/modprobe.d/*.conf) and run modprobe [-r] udf";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux//etc/modprobe.d/ERROR") || get_kb_item("Policy/linux/lsmod/ERROR")){
  ...
  foreach file (get_kb_list("Policy/linux//etc/modprobe.d")){
    ...
    if(egrep(string:file_content, pattern:"^\s*install udf /bin/true"))
      deactivated = TRUE;
  }
