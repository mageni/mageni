...
# $Id: win_spotlight_third_party.nasl 10989 2018-08-15 14:57:51Z emoss $
#
# Check value for Do not suggest third-party content in Windows spotlight
# (users listed in HKU)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312238");
...
'Do not suggest third-party content in Windows spotlight' on Windows hosts (at
least Windows 10).

The setting determines whether Windows will suggest apps and content from third-
...
foreach uid (uids){
  if(uid =~ "S-1-5-21-"){
      ...
      key = uid + '\\Software\\Policies\\Microsoft\\Windows\\CloudContent';
