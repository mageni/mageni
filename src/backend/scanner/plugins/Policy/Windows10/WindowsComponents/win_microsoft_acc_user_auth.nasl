...
# $Id: win_microsoft_acc_user_auth.nasl 10989 2018-08-15 14:57:51Z emoss $
#
# Check value for Block all consumer Microsoft account user authentication
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312204");
...
'Block all consumer Microsoft account user authentication' on Windows hosts (at
least Windows 10).

The setting prevents all applications and services on the device from new
consumer Microsoft account authentication via the Windows OnlineID and
WebAccountManager APIs.
The setting may not affect applications which have already authenticated until
the authentication cache expires.
The setting does not affect authentication performed directly by the user in
browsers or in apps that use OAuth.");
  exit(0);
}
...
Computer Configuration/Administrative Templates/Windows Components/Microsoft accounts/' + title;
type = 'HKLM';
