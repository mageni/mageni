...
# $Id: win_infrared_monitor_service.nasl 10714 2018-08-01 14:49:06Z emoss $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312291");
  ...
  script_name('Microsoft Windows 10: Service: Infrared monitor service');
...
'Infrared monitor service (irmon)' on Windows hosts (Windows 10).

The service detects other infrared devices that are in range and launches the
file transfer app. If stopped, the service prevents file transfer from working.");
  exit(0);
}
...
This setting only applies to Windows 10 systems.');
  exit(0);
}

title = 'Infrared monitor service';
