...
# $Id: win10_lock_pages_memory.nasl 10649 2018-07-27 07:16:55Z emoss $
#
# Check value for Lock pages in memory (WMI)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312269");
  ...
  script_name('Microsoft Windows 10: Lock pages in memory');
...
accounts can use a process to keep data in physical memory, which prevents the 
computer from paging the data to virtual memory on a disk.

Normally, an application running on Windows can negotiate for more physical 
memory, and in response to the request, the application begins to move the data 
from RAM (such as the data cache) to a disk. When the pageable memory is moved 
to a disk, more RAM is free for the operating system to use.");
  exit(0);
}
