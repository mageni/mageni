...
# $Id: win10_change_timezone.nasl 10649 2018-07-27 07:16:55Z emoss $
#
# Check value for Change the time zone (WMI)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312244");
  ...
  script_name('Microsoft Windows 10: Change the time zone');
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (c) 2018 Greenbone Networks GmbH");
  script_family("Policy");
  script_dependencies("gb_wmi_access.nasl", "smb_reg_service_pack.nasl");
  script_add_preference(name:"Value", type:"entry", value:"Administrators, LOCAL SERVICE, Users");
  script_mandatory_keys("Compliance/Launch");
  script_require_keys("WMI/access_successful");
  script_tag(name: "summary", value: "This policy setting determines which users 
can adjust the time zone that is used by the device for displaying the local 
time, which includes the device's system time plus the time zone offset.");
  exit(0);
}
