...
foreach dir (make_list_unique("/suitecrm", "/SuiteCRM", "/suite", "/crm", http_cgi_dirs(port: port))) {
  install = dir;
  if (dir == "/")
    dir = "";
  ...
  if ('alt="SuiteCRM"' >< res && 'id="admin_options">' >< res) {
    ...
    set_kb_item(name: "salesagility/suitecrm/http/detected", value: TRUE);

    # nb: Used in gb_sugarcrm_detect.nasl
    set_kb_item(name: "salesagility/suitecrm/" + port + "/detected", value: TRUE);
    set_kb_item(name: "salesagility/suitecrm/http/" + port + "/detected", value: TRUE);
