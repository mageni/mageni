...
url = dir + "/wp-content/plugins/yop-poll/readme.txt";
res = http_get_cache(port: port, item: url);

if("=== YOP Poll ===" >< res && "Changelog" >< res) {

  vers = eregmatch(pattern: "= ([0-9.]+) =", string: res);

  if(vers[1] && version_is_less(version: vers[1], test_version: "6.0.3")) {
    report = report_fixed_ver(installed_version: vers[1], fixed_version: "6.0.3", file_checked: url);
