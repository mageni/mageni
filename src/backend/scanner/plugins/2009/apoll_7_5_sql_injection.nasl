...
# $Id: apoll_7_5_sql_injection.nasl 12386 2018-11-16 13:48:49Z cfischer $
#
...
plain_host = http_host_name( dont_add_port:TRUE );
variables = "username=select username from ap_users' or ' 1=1'-- '&email=" + vt_strings["default"] + "@" + plain_host;
...
foreach dir( make_list_unique( "/apoll", "/poll", cgi_dirs( port:port ) ) ) {
  ...
  url = dir + "/admin/lost-pass.php";
  res = http_get_cache( item:url, port:port );
  if( ! res || res !~ "^HTTP/1\.[01] 200" || "You successfully generate new password" >< res )
    continue;
