...
# $Id: gb_solarwinds_ipam_detect_win.nasl 13827 2019-02-22 07:50:44Z mmartin $
#
...
if(description)
{
  ...
  script_name("SolarWinds IP Address Manager Version Detection (Windows)");
  ...
  of SolarWinds IP Address Manager for Windows.");
...
foreach key (key_list) {
  foreach item (registry_enum_keys(key:key)) {
    ...
    if(!appName || appName !~ "SolarWinds IP Address Manager") continue;
    version = "unknown";
    concluded += "SolarWinds IPAM";
    ...
    set_kb_item(name:"solarwinds/ipam/win/detected", value:TRUE);
