...
if(description)
{
  ...
  script_name("Nakid CMS Detection (HTTP");
  ...
  script_tag(name:"summary", value:"HTTP based detection of Nakid CMS.");

  script_xref(name:"URL", value:"https://github.com/kilrizzy/Nakid-CMS");
...
foreach dir( make_list_unique( "/nakid", "/Nakid", "/", http_cgi_dirs( port:port ) ) ) {
  ...
  if( ">Nakid CMS<" >< res ) {
    ...
    set_kb_item( name:"nakidcms/detected", value:TRUE );
    set_kb_item( name:"nakidcms/http/detected", value:TRUE );
    ...
    log_message( data:build_detection_report( app:"Nakid CMS", version:version, install:install,
