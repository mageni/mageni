...
# $Id: gb_nmap_snmp_brute_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104079");
  ...
  script_name("Nmap NSE net: snmp-brute");
...
snmplist:  The filename of a list of community strings to try.
...
if (phase == 1) {
    ...
    pref = script_get_preference("snmplist");
    if (!isnull(pref) && pref != "") {
        argv["snmplist"] = string('"', pref, '"');
    }
    pref = script_get_preference("snmpcommunity");
    if (!isnull(pref) && pref != "") {
        argv["snmpcommunity"] = string('"', pref, '"');
    }
    nmap_nse_register(script:"snmp-brute", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"snmp-brute");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (snmp-brute.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
