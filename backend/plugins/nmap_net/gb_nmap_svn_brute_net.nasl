...
# $Id: gb_nmap_svn_brute_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  ...
  script_name("Nmap NSE net: svn-brute");
  ...
  script_add_preference(name:"svn-brute.force", value:"", type:"entry");
  ...
  script_add_preference(name:"svn-brute.repo", value:"", type:"entry");
...
if (phase == 1) {
    ...
    pref = script_get_preference("svn-brute.force");
    if (!isnull(pref) && pref != "") {
        argv["svn-brute.force"] = string('"', pref, '"');
    }
    ...
    pref = script_get_preference("svn-brute.repo");
    if (!isnull(pref) && pref != "") {
        argv["svn-brute.repo"] = string('"', pref, '"');
    }
    pref = script_get_preference("brute.threads");
    if (!isnull(pref) && pref != "") {
        argv["brute.threads"] = string('"', pref, '"');
    }
    nmap_nse_register(script:"svn-brute", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"svn-brute");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (svn-brute.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
