...
if(description)
{
  ...
  of Rockwell Automation FactoryTalk Alarms and Events for Windows.");
...
foreach key (key_list) {
  foreach item (registry_enum_keys(key:key)) {
    ...
    if(!appName || appName !~ "FactoryTalk Alarms and Events") continue;
    ...
    set_kb_item(name:"rockwellautomation/factorytalk_alarms_and_events/win/detected", value:TRUE);

    register_and_report_cpe(app:"Rockwell Automation " +appName , ver:version, concluded:concluded,
                          base:"cpe:/a:rockwellautomation:factorytalk_alarms_and_events:", expr:"^([0-9.]+)", insloc:location, regService:"smb-login", regPort:0);
    exit(0);
  }
}
