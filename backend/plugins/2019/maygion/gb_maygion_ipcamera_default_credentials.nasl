...
# $Id: gb_maygion_ipcamera_default_credentials.nasl 13469 2019-02-05 12:31:12Z tpassfeld $
#
# MayGion IPCamera Default Credentials
#
# Authors:
...
if(description)
{
  ...
  script_name("MayGion IPCamera Default Credentials");
  ...
  script_tag(name:"summary", value:"The remote installation of MayGion's IP camera software is prone to
  a default account authentication bypass vulnerability.");

  script_tag(name:"impact", value:"This issue may be exploited by a remote attacker to gain access
  to sensitive information or modify system configuration.");

  script_tag(name:"insight", value:"The installation of MayGion's IP camera software is lacking a proper
...
foreach cred(keys(creds)) {
  ...
  url = "/login.xml?user=" + cred + "&usr=" + cred + "&password=" + creds[cred] + "&pwd=" + creds[cred];

  req = http_get_req(port: port, url: url, user_agent: TRUE, add_headers: make_array("Cookie", "bRememberMe=0; userLastLogin=; passwordLastLogin="));
