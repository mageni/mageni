...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150283");
  ...
  script_name("Huawei Data Communication: Read and parse display mpls ldp session verbose");
  ...
  script_tag(name:"summary", value:"Get mpls ldp session configuration of the VRP device.
...
if(!ret){
  set_kb_item(name:"Policy/vrp/mpls/ldp/session/ERROR", value:TRUE);
  exit(0);
}

set_kb_item(name:"Policy/vrp/mpls/ldp/session", value:ret);
