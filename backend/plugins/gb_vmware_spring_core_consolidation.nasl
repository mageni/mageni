...
if(description)
{
  ...
  script_name("VMware Spring Framework (Core) Detection Consolidation");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2022 Greenbone Networks GmbH");
  script_family("Product detection");
  script_dependencies("gb_vmware_spring_core_ssh_login_detect.nasl");
  script_mandatory_keys("vmware/spring/framework/core/detected");

  script_tag(name:"summary", value:"Consolidation of VMware Spring Framework (Core component)
  detections.");
...
if( ! get_kb_item( "vmware/spring/framework/core/detected" ) )
  exit( 0 );
...
foreach source( make_list( "ssh-login", "smb-login" ) ) {
  ...
  install_list = get_kb_list( "vmware/spring/framework/core/" + source + "/*/installs" );
  if( ! install_list )
    continue;
  ...
  foreach install( install_list ) {
    ...
    cpe = build_cpe( value:version, exp:"^([0-9.x]+)", base:"cpe:/a:vmware:spring_framework_core:" );
    if( ! cpe )
    ...
    if( report )
      report += '\n\n';
    report += build_detection_report( app:"VMware Spring Framework (Core)",
                                      version:version,
                                      install:install,
                                      cpe:cpe,
                                      concluded:concl );
