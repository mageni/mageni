...
# $Id: win_timout_screen_saver.nasl 10989 2018-08-15 14:57:51Z emoss $
#
# Check value for Screen saver timeout (users listed in HKU)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312505");
  script_version("$Revision: 10989 $");
  script_tag(name:"last_modification", value:"$Date: 2018-08-15 16:57:51 +0200 (Wed, 15 Aug 2018) $");
  script_tag(name:"creation_date", value:"2018-06-28 16:05:24 +0200 (Thu, 28 Jun 2018)");
  script_tag(name:"cvss_base", value:"0.0");
  script_tag(name:"cvss_base_vector", value:"AV:L/AC:H/Au:S/C:N/I:N/A:N");
  script_tag(name:"qod", value:"97");
  script_name('Microsoft Windows: Screen saver timeout');
...
'Screen saver timeout' on Windows hosts (at least Windows 7).

The setting specifies how much user idle time must elapse before the screen
saver is launched.
When configured, the idle time can be set from a minimum of 1 second to a
maximum of 86, 400 seconds (24 hours).
If set to zero, the screen saver is not started at all.");
  exit(0);
}
...
foreach entry (keys(v)){
  if(v[entry] == '0' && default != '0'){
    compliant = 'no';
  }else if(v[entry] <= default && v[entry] == '0'){
    compliant = 'no';
  }
}
