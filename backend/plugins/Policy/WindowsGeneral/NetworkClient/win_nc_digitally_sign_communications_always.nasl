...
# $Id: win_nc_digitally_sign_communications_always.nasl 10740 2018-08-02 14:13:50Z emoss $
#
# Check value for Microsoft network client: Digitally sign communications (always)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312594");
  ...
  script_name('Microsoft Windows: Microsoft network client: Digitally sign communications (always)');
...
'Microsoft network client: Digitally sign communications (always)' on Windows
...
attacks. If enabled, the SMB client will only communicate with an SMB server
that performs SMB packet signing. ");
  exit(0);
}
