...
# $Id: win_iis_admin_service.nasl 10714 2018-08-01 14:49:06Z emoss $
#
# Check value for IIS Admin Service (IISADMIN)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312673");
  ...
  script_name('Microsoft Windows: Service: IIS Admin Service');
...
'IIS Admin Service (IISADMIN)' on Windows hosts (at least Windows 8.1).

The service enables the server to administer the IIS metabase, which stores
configuration for the SMTP and FTP services.
If stopped, the server is unable to configure SMTP or FTP.");
  exit(0);
}
...
This setting applies to at least Windows 8.1 systems.');
  exit(0);
}

title = 'IIS Admin Service';
