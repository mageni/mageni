...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312408");
...
'Do not use temporary folders per session' on Windows hosts (at least Windows 7).

The setting prevents Remote Desktop Services from creating session-specific
temporary folders.
The setting disables the creation of separate temporary folders on a remote
computer for each session.");
  exit(0);
}
