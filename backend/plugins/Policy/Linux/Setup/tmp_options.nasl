...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.109722");
  ...
  script_name("Linux: Check options for /tmp directory");
...
This script tests options set on /tmp filesystem.");
...
cmd = "mount | grep /tmp";
title = "Mount options on /tmp";
solution = "mount -o remount,[OPTIONS] /tmp";
test_type = "SSH_Cmd";
...
if(get_kb_item("linux/mount/ERROR")){
  ...
  value = get_kb_item("linux/mount//tmp/options");
