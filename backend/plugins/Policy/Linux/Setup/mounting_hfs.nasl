...
if(description)
{
  ...
  script_tag(name:"summary", value:"The hfs filesystem type is a filesystem that allows to mount
Mac OS filesystems.

Linux kernel modules which implement filesystems that are not needed by the local system should be
disabled.

Note: This script looks for entry 'install hfs /bin/true' in files in /etc/modprobe.d/*.conf and
...
solution = "Add or remove 'install hfs /bin/true' in config file (/etc/modprobe.d/*.conf) and run modprobe [-r] hfs";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux//etc/modprobe.d/ERROR") || get_kb_item("Policy/linux/lsmod/ERROR")){
  ...
  foreach file (get_kb_list("Policy/linux//etc/modprobe.d")){
    ...
    if(egrep(string:file_content, pattern:"^\s*install hfs /bin/true"))
      deactivated = TRUE;
  }
