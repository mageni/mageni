...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.109726");
  ...
  script_name("Linux: Separate partition for /var/log/audit directory");
...
This script tests options set on /var/log/audit filesystem.");
...
title = "Mount options on /var/log/audit";
solution = "mount -o remount,[OPTIONS] /var/log/audit";
test_type = "SSH_Cmd";
...
if(get_kb_item("linux/mount/ERROR")){
  ...
  value = get_kb_item("linux/mount//var/log/audit/options");
