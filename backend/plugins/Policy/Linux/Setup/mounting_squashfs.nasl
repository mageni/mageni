...
if(description)
{
  ...
  script_tag(name:"summary", value:"The squashfs filesystem type is embedded in small footprint
systems.

Linux kernel modules which implement filesystems that are not needed by the local system should be
disabled.

Note: This script looks for entry 'install squashfs /bin/true' in files in /etc/modprobe.d/*.conf and
...
cmd = "lsmod; grep -r 'install squashfs /bin/true' /etc/modprobe.d/*";
title = "Mounting of squashfs filesystems";
solution = "Add or remove 'install squashfs /bin/true' in config file (/etc/modprobe.d/*.conf) and run modprobe [-r] squashfs";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux//etc/modprobe.d/ERROR") || get_kb_item("Policy/linux/lsmod/ERROR")){
  ...
  foreach file (get_kb_list("Policy/linux//etc/modprobe.d")){
    ...
    if(egrep(string:file_content, pattern:"^\s*install squashfs /bin/true"))
      deactivated = TRUE;
  }
