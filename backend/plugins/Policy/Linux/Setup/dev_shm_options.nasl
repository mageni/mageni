...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.109728");
  ...
  script_name("Linux: Check options for /dev/shm directory");
...
This script tests options set on /dev/shm filesystem.");
  exit(0);
}
...
title = "Mount options on /dev/shm";
solution = "mount -o remount,[OPTIONS] /dev/shm";
test_type = "SSH_Cmd";
...
if(get_kb_item("linux/mount/ERROR")){
  ...
  value = get_kb_item("linux/mount//dev/shm/options");
