...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150064");
  ...
  script_name("Linux: Separate partition for /var/tmp");
...
This script tests if a separate partition exists for /var/tmp.");
...
title = "Separate partition for /var/tmp";
solution = "Create a new partition and configure /var/tmp as appropriate";
test_type = "SSH_Cmd";
...
if(get_kb_item("linux/mount/ERROR")){
  ...
  if(!in_array(search:"/var/tmp", array:mounted_partitions, part_match:FALSE))
    value = "no";
  else
    value = "yes";
