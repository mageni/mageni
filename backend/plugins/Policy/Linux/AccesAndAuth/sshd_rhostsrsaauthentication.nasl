...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150224");
  ...
  script_name("Linux: SSH RhostsRSAAuthentication");
...
enclosed in double quotes in order to represent arguments containing spaces.
  ...
  - RhostsRSAAuthentication: Specifies whether rhosts or /etc/hosts.equiv authentication together with
successful RSA host authentication is allowed. This option applies to protocol version 1 only.");
...
cmd = "grep '^RhostsRSAAuthentication' /etc/ssh/sshd_config";
title = "SSH RhostsRSAAuthentication";
...
if(get_kb_item("linux/mount/ERROR")){
  ...
  value = get_kb_item("Policy/linux/sshd_config/rhostsrsaauthentication");
