...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150229");
  ...
  script_name("Linux: SSH AllowAgentForwarding");
...
enclosed in double quotes in order to represent arguments containing spaces.
  ...
  - AllowAgentForwarding: Specifies whether ssh-agent forwarding is permitted. Note that disabling
...
cmd = "grep '^AllowAgentForwarding' /etc/ssh/sshd_config";
title = "SSH AllowAgentForwarding";
...
if(get_kb_item("linux/mount/ERROR")){
  ...
  value = get_kb_item("Policy/linux/sshd_config/allowagentforwarding");
