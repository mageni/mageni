...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150231");
  ...
  script_name("Linux: SSH PermitTunnel");
  ...
  script_add_preference(name:"Value", type:"radio", value:"no;yes;point-to-point;ethernet", id:1);
...
enclosed in double quotes in order to represent arguments containing spaces.
  ...
  - PermitTunnel: Specifies whether tun device forwarding is allowed. The argument must be 'yes',
...
cmd = "grep '^PermitTunnel' /etc/ssh/sshd_config";
title = "SSH PermitTunnel";
...
if(get_kb_item("linux/mount/ERROR")){
  ...
  value = get_kb_item("Policy/linux/sshd_config/permittunnel");
