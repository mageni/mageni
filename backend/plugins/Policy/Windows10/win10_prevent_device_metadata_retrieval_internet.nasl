...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312282");
  ...
  script_name('Microsoft Windows 10: Prevent device metadata retrieval from the Internet');
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (c) 2018 Greenbone Networks GmbH");
  script_family("Policy");
  script_dependencies("smb_reg_service_pack.nasl");
  script_mandatory_keys("Compliance/Launch");
  script_tag(name: "summary", value: "This policy setting prevents Windows from 
retrieving device metadata from the Internet.

If enabled, Windows does not retrieve device metadata for installed devices from the Internet.
