...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150298");
  ...
  script_name("Huawei Data Communication: Configuring the Function of Checking DHCP Renewal Packet Attacks");
  ...
  script_tag(name:"summary", value:"This function is used to prevent attackers from attacking the DHCP
server by sending forged DHCP packets for extending the lease.");
...
title = "Configuring the Function of Checking DHCP Renewal Packet Attacks";
solution = "Configure the user binding table check function when lease renewal packet attacks occur.
DHCP snooping checks DHCP packets against the binding table.";
test_type = "SSH_Cmd";
default = "Enabled";
...
if(get_kb_item("Policy/vrp/installed/ERROR")){
  ...
  if(current_configuration =~ "dhcp\s+snooping\s+check\s+dhcp-request\s+enable")
    value = "Enabled";
  else
    value = "Disabled";
