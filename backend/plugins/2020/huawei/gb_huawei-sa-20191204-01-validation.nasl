...
if (description)
{
  ...
  script_tag(name:"summary", value:"Multiple Huawei products are prone to a denial of service vulnerability due
  to insufficient verification of data authenticity.");

  script_tag(name:"insight", value:"A remote, unauthenticated attacker has to intercept specific packets between
  two devices, modifies the packets, and sends the modified packets to the peer device. Due to insufficient
  verification of some fields in the packets, an attacker may exploit the vulnerability to cause the target device
  abnormal.");
  ...
  script_tag(name:"affected", value:"Huawei AR150, AR160, AR200, AR1200, AR2200, AR3200, AR3600, S6700, SRG1300,
  SRG2300 and SRG3300.");
...
if (cpe == "cpe:/o:huawei:s6700_firmware") {
  if (version_is_greater_equal(version: version, test_version: "v200r008c00") &&
    ...
    report = report_fixed_ver(installed_version: toupper(version), fixed_version: "V200R11C10");
    security_message(port: 0, data: report);
    exit(0);
  }
} else {
  if (version_is_greater_equal(version: version, test_version: "v200r005c20") &&
      version_is_less(version: version, test_version: "v200r009c00spc500")) {
    report = report_fixed_ver(installed_version: toupper(version), fixed_version: "V200R009C00SPC500");
    security_message(port: 0, data: report);
    exit(0);
  }
}
