...
if(description)
{
  ...
  script_name("Sophos XG Firewall Detection (HTTP, Admin Portal)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of a Sophos XG Firewall from the admin
  portal.");
...
if( res1 !~ "Sophos [^ ]*Firewall" )
  exit( 0 );

set_kb_item( name:"sophos/xg_firewall/detected", value:TRUE );
set_kb_item( name:"sophos/xg_firewall/http-admin/detected", value:TRUE );
set_kb_item( name:"sophos/xg_firewall/http-admin/port", value:port );
...
if( ! isnull( vers[1] ) ) {
  version = vers[1];
  set_kb_item( name:"sophos/xg_firewall/http-admin/" + port + "/concluded", value:vers[0] );
  set_kb_item( name:"sophos/xg_firewall/http-admin/" + port + "/concludedUrl",
               value:http_report_vuln_url( port:port, url:url, url_only:TRUE ) );
}

set_kb_item( name:"sophos/xg_firewall/http-admin/" + port + "/version", value:version );
