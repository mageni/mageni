...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.811279");
...
foreach item (registry_enum_keys(key:ms_sql_key))
{
  ...
  if("Microsoft SQL Server" >< sql_path)
  {
    ...
    if(sysVer =~ "^12\.0")
    {
      if(version_in_range(version:sysVer, test_version:"12.0.5000.0", test_version2:"12.0.5206.0"))
      {
        report = 'File checked:     ' + sql_path + "\microsoft.sqlserver.chainer.infrastructre.dll" + '\n' +
                 'File version:     ' + sysVer  + '\n' +
                 'Vulnerable range: 12.0.5000.0 - 12.0.5206.0\n' ;
