...
#
# Microsoft SQL Server 2012 Service Pack 3 Information Disclosure Vulnerability (KB4019090)
#
# Authors:
...
foreach item (registry_enum_keys(key:ms_sql_key))
{
  ...
  if("Microsoft SQL Server" >< sql_path)
  {
    ...
    if(sysVer =~ "^11\.0")
    {
      if(version_in_range(version:sysVer, test_version:"11.0.6400.0", test_version2:"11.0.6607.2"))
      {
        report = 'File checked:     ' + sql_path + "\microsoft.sqlserver.chainer.infrastructre.dll" + '\n' +
                 'File version:     ' + sysVer  + '\n' +
                 'Vulnerable range: 11.0.6400.0 - 11.0.6607.2\n' ;
