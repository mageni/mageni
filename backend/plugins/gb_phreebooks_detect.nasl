...
if(description)
{
  ...
  script_name("PhreeBooks Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of PhreeBooks.");
...
foreach dir (make_list_unique("/", "/phreeBooks", "/phreebooks", "/pb", http_cgi_dirs(port: port))) {
 install = dir;
 if (dir == "/")
   dir = "";
 ...
 if (egrep(pattern: "<title>PhreeBooks</title>", string: res, icase: TRUE)) {
    version = "unknown";
    ...
    log_message(data: build_detection_report(app: "PhreeBooks", version: version, install: install,
                                             cpe: cpe),
                port: port);
    exit(0);
  }
