...
if(description)
{
 script_oid("1.3.6.1.4.1.25623.1.0.306022");
 ...
 script_name("FreeBSD Ports: ganglia-monitor-core, ganglia-monitor-webfrontend");
...
if(!isnull(bver) && revcomp(a:bver, b:"3.1.1")<0) {
    txt += 'Package ganglia-monitor-core version ' + bver + ' is installed which is known to be vulnerable.\n';
    vuln = 1;
}
bver = portver(pkg:"ganglia-monitor-webfrontend");
if(!isnull(bver) && revcomp(a:bver, b:"3.1.1")<0) {
    txt += 'Package ganglia-monitor-webfrontend version ' + bver + ' is installed which is known to be vulnerable.\n';
    vuln = 1;
}
