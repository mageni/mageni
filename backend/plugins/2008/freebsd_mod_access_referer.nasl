...
tag_insight = "The following package is affected: mod_access_referer

CVE-2003-1054
mod_access_referer 1.0.2 allows remote attackers to cause a denial of
service (crash) via a malformed Referer header that is missing a
hostname, as parsed by the ap_parse_uri_components function in Apache,
...
if(description)
{
 script_oid("1.3.6.1.4.1.25623.1.0.303078");
 ...
 script_name("FreeBSD Ports: mod_access_referer");
...
if(!isnull(bver) && revcomp(a:bver, b:"1.0.2_1")<0) {
    txt += 'Package mod_access_referer version ' + bver + ' is installed which is known to be vulnerable.\n';
    vuln = 1;
}
