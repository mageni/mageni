...
tag_insight = "VNC servers created with x11vnc accept insecure protocol types, even when
the server does not offer it, resulting in the possibility of unauthorized
access to the server.";
tag_solution = "All x11vnc users should upgrade to the latest version:

    # emerge --sync
    # emerge --ask --oneshot --verbose '>=x11-misc/x11vnc-0.8.1'

http://www.securityspace.com/smysecure/catid.html?in=GLSA%20200608-12
http://bugs.gentoo.org/show_bug.cgi?id=142559
...
if(description)
{
 script_oid("1.3.6.1.4.1.25623.1.0.301931");
