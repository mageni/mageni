...
if(description)
{
  ...
  script_tag(name:"summary", value:"Checks whether GoAhead is present on
...
if( buf =~ 'Server: *GoAhead-Webs' ) {
  set_kb_item( name: "embedthis/goahead/detected", value: TRUE );

  version = "unknown";

  ver = eregmatch( string: buf, pattern: 'GoAhead-Webs/([0-9.]+)' );
  if( ! isnull( ver[1] ) )
    version = ver[1];

  register_and_report_cpe( app: "GoAhead",
                           ver: version,
                           concluded: ver[0],
                           base: CPE,
                           expr: "([0-9.]+)",
