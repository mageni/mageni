...
if(description)
{
  ...
  script_name("SolarWinds Server & Application Monitor Version Detection (Windows)");
  ...
  of SolarWinds Server & Application Monitor for Windows.");
...
foreach key (key_list) {
  foreach item (registry_enum_keys(key:key)) {
    ...
    set_kb_item(name:"solarwinds/sam/win/detected", value:TRUE);
    set_kb_item(name:"solarwinds/sam/win/ver", value:version);

    register_and_report_cpe(app:appName, ver:version, concluded:concluded,
    base:"cpe:/a:solarwinds:server_application_monitor:", expr:"^([0-9.]+)", insloc:location, regService:"smb-login", regPort:0);
