...
if(description)
{
  ...
  script_name("Caucho Resin Detection");
  ...
  script_tag(name:"summary", value:"Checks whether Resin is present on the
...
if( buf =~ 'Server: *Resin' ) {
  ...
  ver = eregmatch( string: buf, pattern: 'Resin/([0-9.]+)' );
  if( ! isnull( ver[1] ) )
    version = ver[1];

  register_and_report_cpe( app: "Resin",
                           ver: version,
                           concluded: ver[0],
                           base: CPE,
                           expr: "([0-9.]+)",
