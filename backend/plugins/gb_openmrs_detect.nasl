...
if (description)
{
  ...
  script_xref(name:"URL", value:"https://openmrs.org/");
...
foreach dir (make_list_unique("/openmrs", cgi_dirs(port: port))) {
  install = dir;
  ...
  if (("OPENMRS_CONTEXT_PATH" >< res && "referenceapplication/home.page" >< res) ||
    ...
    # < 2.x
    # "codeVersion">Version: 1.9.0  Build 27760
    vers = eregmatch(pattern: '"codeVersion">Version: ([0-9.]+)', string: res);
    if (!isnull(vers[1]))
      version = vers[1];

    set_kb_item(name:"openmrs/detected", value: TRUE);

    cpe = build_cpe(value: version, exp: "^([0-9.]+)", base: "cpe:/a:openmrs:openmrs:");
    if (!cpe)
      cpe = 'cpe:/a:openmrs:openmrs';

    register_product(cpe: cpe, location: install, port: port, service: "www");

    log_message(data: build_detection_report(app: "OpenMRS", version: version, install: install, cpe: cpe,
                                             concluded: vers[0]),
                port: port);
    exit(0);
  }
