...
if (description)
{
  ...
  script_name("Apache HTTP Server DoS Vulnerability (Jan 2012) - Linux");
  ...
  script_tag(name:"insight", value:"A flaw was found when mod_proxy_ajp connects to a backend
  server that takes too long to respond. Given a specific configuration, a remote attacker could
  send certain requests, putting a backend server into an error state until the retry timeout
...
if (version_in_range(version: version, test_version: "2.2.12", test_version2: "2.2.21")) {
