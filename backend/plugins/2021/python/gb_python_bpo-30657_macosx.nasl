...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.118246");
  ...
  script_name("Python < 2.7.14, 3.4.x < 3.4.8, 3.5.x < 3.5.5 PyString_DecodeEscape integer overflow (bpo-30657) - Mac OS X");
  ...
  script_tag(name:"summary", value:"CPython (aka Python) is prone to an integer overflow
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"An integer overflow in the 'PyString_DecodeEscape' function in
  'stringobject.c' is resulting in a heap-based buffer overflow (and possible arbitrary code
  execution).");

  script_tag(name:"affected", value:"Python prior to version 2.7.14, versions 3.4.x prior to 3.4.8
  and 3.5.x prior to 3.5.5.");

  script_tag(name:"solution", value:"Update to version 2.7.14, 3.4.8, 3.5.5 or later.");

  script_xref(name:"URL", value:"https://python-security.readthedocs.io/vuln/pystring_decodeescape-integer-overflow.html");
  script_xref(name:"Advisory-ID", value:"bpo-30657");
