...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2021.2869");
  ...
  script_name("Huawei EulerOS: Security Advisory for glib2 (EulerOS-SA-2021-2869)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2021-2869");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2021-2869");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'glib2' package(s) announced via the EulerOS-SA-2021-2869 advisory.");
...
** DISPUTED ** GNOME GLib before 2.65.3 has an integer overflow, that might lead to an out-of-bounds write, in g_option_group_add_entries. NOTE: the vendor's position is 'Realistically this is not a security issue. The standard pattern is for callers to provide a static list of option entries in a fixed number of calls to g_option_group_add_entries().' The researcher states that this pattern is undocumented.(CVE-2020-35457)");
  ...
  script_tag(name:"affected", value:"'glib2' package(s) on Huawei EulerOS Virtualization 3.0.2.6.");
...
if(release == "EULEROSVIRT-3.0.2.6") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"glib2", rpm:"glib2~2.54.2~2.h8.eulerosv2r7", rls:"EULEROSVIRT-3.0.2.6"))) {
    report += res;
  }

  if(!isnull(res = isrpmvuln(pkg:"glib2-devel", rpm:"glib2-devel~2.54.2~2.h8.eulerosv2r7", rls:"EULEROSVIRT-3.0.2.6"))) {
    report += res;
  }
