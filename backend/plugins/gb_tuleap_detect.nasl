...
# $Id: gb_tuleap_detect.nasl 11885 2018-10-12 13:47:20Z cfischer $
#
# Tuleap Detection
#
# Authors:
...
if (description)
{
  ...
  script_name("Tuleap Detection");

  script_tag(name:"summary", value:"Detection of Tuleap

The script sends a connection request to the server and attempts to detect the presence of Tuleap and to
extract its version");
...
if ("<title>Welcome - Tuleap" >< res && "/account/login.php" >< res) {
  ...
  set_kb_item(name: "tuleap/installed", value: TRUE);
  ...
  log_message(data: build_detection_report(app: "Tuleap", version: version, install: "/", cpe: cpe,
                                           concluded: vers[0]),
              port: port);
  exit(0);
}
