...
if(description)
{
  ...
  script_version("2021-03-04T02:09:53+0000");
  ...
  script_name("HP Systems Insight Manager Detection (HTTP)");

  script_tag(name:"summary", value:"HTTP based detection of HP Systems Insight Manager.");
...
if ("<title>HP Systems Insight Manager" >< res && 'signInTitle"><h1>HP Systems Insight Manager' >< res) {
  version = "unknown";
  ...
  log_message(data: build_detection_report(app: "HP Systems Insight Manager", version: version, install: "/",
                                           cpe: cpe),
              port: port);
  exit(0);
}
