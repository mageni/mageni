...
if(description)
{
  ...
  script_name("Openswan Detection (Linux/Unix SSH Login)");
  ...
  script_tag(name:"summary", value:"SSH login-based detection of Openswan.");
...
foreach bin (paths) {
  ...
  if (!isnull(vers[1])) {
    set_kb_item(name: "openswan/detected", value: TRUE);
    set_kb_item(name: "openswan/ssh-login/detected", value: TRUE);
    ...
    cpe = build_cpe(value: version, exp: "^([0-9.]+)(rc[0-9])?", base: "cpe:/a:openswan:openswan:");
    if (!cpe)
      cpe = "cpe:/a:openswan:openswan";

    register_product(cpe: cpe, location: bin, port: 0, service: "ssh-login");

    log_message(data: build_detection_report(app: "Openswan", version: version, install: bin,
                                             concluded: vers[0]),
                port: 0);
  }
}
