...
# $Id: gb_orion_sam_detect.nasl 10929 2018-08-11 11:39:44Z cfischer $
#
...
if (description)
{
  ...
  script_name("SolarWinds Orion Server and Application Monitor Detection");
...
if ("SolarWinds Platform" >< buf || "SolarWinds Orion" >< buf || "Orion Platform" >< buf) {
  sam = eregmatch(string:buf, pattern: "SAM ([0-9.]+)", icase:TRUE);
  if (!isnull(sam)) {
    vers = string("unknown");
    if (!isnull(sam[1])) {
      vers=chomp(sam[1]);
    ...
    set_kb_item(name: string("www/", port, "/orion_sam"), value: vers);
    set_kb_item(name: "orion_sam/installed", value: TRUE);

    cpe = build_cpe(value: vers, exp:"^([0-9.]+)", base: "cpe:/a:solarwinds:server_and_application_monitor:");
    if (isnull(cpe))
      cpe = 'cpe:/a:solarwinds:server_and_application_monitor';

    register_product(cpe:cpe, location:dir, port:port);

    log_message(data: build_detection_report(app:"SolarWinds Server and Application Monitor", version: vers,
                                             install: dir, cpe: cpe,
                                             concluded: sam[0]), port:port);
  }
