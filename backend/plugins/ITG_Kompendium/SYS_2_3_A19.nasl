...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150606");
...
verschluesselt werden. Die dazugehoerigen Schluessel SOLLTEN NICHT auf dem IT-System gespeichert
werden. Es SOLLTEN AEAD-Verfahren (Authenticated Encryption with Associated Data) bei der
Festplatten- und Dateiverschluesselung eingesetzt werden. Alternativ SOLLTE 'dm-crypt' in
Kombination mit 'dm-verity' genutzt werden.");
...
title = "Festplatten- oder Dateiverschluesselung";

if (host_runs("linux") != "yes"){
  result = itg_result_wrong_target();
  desc = itg_desc_wrong_target();
  itg_set_kb_entries(result:result, desc:desc, title:title, id:"SYS.2.3.A19");
  exit(0);
}
