...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150600");
  ...
  script_tag(name:"summary", value:"Es SOLLTEN Quotas fuer Benutzer bzw. Dienste eingerichtet werden,
die ausreichend Freiraum fuer das Betriebssystem lassen. Generell SOLLTEN unterschiedliche Partitionen
fuer Betriebssystem und Daten genutzt werden. Alternativ SOLLTEN auch Mechanismen des verwendeten
Dateisystems genutzt werden, die ab einem geeigneten Fuellstand nur noch dem Benutzer 'root'
Schreibrechte einraeumen.");
...
title = "Verhinderung der Ueberlastung der lokalen Festplatte";

if (host_runs("linux") != "yes"){
  result = itg_result_wrong_target();
  desc = itg_desc_wrong_target();
  itg_set_kb_entries(result:result, desc:desc, title:title, id:"SYS.2.3.A11");
  exit(0);
}
