...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150596");
  ...
  script_tag(name:"summary", value:"Wechsellaufwerke SOLLTEN NICHT automatisch eingebunden werden.
Die Einbindung von Wechsellaufwerken SOLLTE so konfiguriert sein, dass alle Dateien als nicht
ausfuehrbar markiert sind (Mountoption 'noexec').");
...
Automounting ist deaktiviert.";

oid_list = make_list("1.3.6.1.4.1.25623.1.0.109730");

if (host_runs("linux") != "yes"){
  result = itg_result_wrong_target();
  desc = itg_desc_wrong_target();
  itg_set_kb_entries(result:result, desc:desc, title:title, id:"SYS.2.3.A6");
  exit(0);
}
