...
if (description)
{
  ...
  script_tag(name:"summary", value:"Reports the Toshiba Printer model and firmware version.");
...
os_name = "Toshiba Printer ";
hw_name = os_name;
if (detected_model != "unknown") {
  os_name += detected_model + " Firmware";
  hw_name += detected_model;
  cpe_model = tolower(detected_model);
  hw_cpe = "cpe:/h:toshibatec:" + cpe_model;
  if (detected_fw_version != "unknown")
    os_cpe = build_cpe(value: tolower(detected_fw_version), exp: "^([0-9a-z.-]+)",
                       base: "cpe:/o:toshibatec:" + cpe_model + "_firmware:");
  else
    os_cpe = "cpe:/o:toshibatec:" + cpe_model + "_firmware";
} else {
  os_name = "Toshiba Printer Unknown Model Firmware";
  hw_name = "Toshiba Printer Unknown Model";
  hw_cpe = "cpe:/h:toshibatec:printer";
  if (detected_fw_version != "unknown")
    os_cpe = build_cpe(value: tolower(detected_fw_version), exp: "^([0-9a-z.-]+)",
                       base: "cpe:/o:toshibatec:printer_firmware:");
  else
    os_cpe = "cpe:/o:toshibatec:printer_firmware";
}
...
register_and_report_os(os: os_name, cpe: os_cpe,
                       desc: "Toshiba Printer Detection Consolidation", runs_key: "unixoide");
