...
foreach key (key_list)
{
  foreach item (registry_enum_keys(key:key))
  {
    ...
    if("Adobe Acrobat Reader DC" >< adobeName && "Acrobat Reader DC" >< adobePath)
    {
      ...
      if(adobeVer)
      {
        set_kb_item(name:"Adobe/Acrobat/ReaderDC/Continuous/Win/Ver", value:adobeVer);

        ## New cpe created
        cpe = build_cpe(value:adobeVer, exp:"^([0-9.]+)", base:"cpe:/a:adobe:acrobat_reader_dc_continuous:");
        if(!cpe)
          cpe = "cpe:/a:adobe:acrobat_reader_dc_continuous";

        if("64" >< os_arch && "Wow6432Node" >!< key)
        {
          set_kb_item(name:"Adobe/Acrobat/ReaderDC/Continuous64/Win/Ver", value:adobeVer);
          cpe = build_cpe(value:adobeVer, exp:"^([0-9.]+)", base:"cpe:/a:adobe:acrobat_reader_dc_continuous:x64:");
          if(!cpe)
            cpe = "cpe:/a:adobe:acrobat_reader_dc_continuous:x64";
        }

        register_product(cpe:cpe, location:adobePath);
        log_message(data: build_detection_report(app:"Adobe Acrobat Reader DC (Continuous Track)", version: adobeVer,
