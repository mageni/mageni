...
if(description)
{
  ...
  script_name("WD My Book Live Detection");

  script_tag(name:"summary", value:"Detection of WD My Book Live.

The script sends a connection request to the server and attempts to detect WD My Book Live and to extract its
version.");
...
res = http_get_cache(port: port, item: "/UI/");

if ("<title>MY BOOK&reg; LIVE&trade;</title>" >< res && "device_rebooting_text" >< res) {
  version = "unknown";
  ...
  if (!cpe)
    cpe = 'cpe:/h:western_digital:my_book_live';

  register_product(cpe: cpe, location: "/", port: port, service: "www");

  log_message(data: build_detection_report(app: "WD My Book Live", version: version, install: "/", cpe: cpe,
