...
# $Id: gb_manageengine_oputils_detect.nasl 10911 2018-08-10 15:16:34Z cfischer $
#
...
if(description)
{
  ...
  script_name("ManageEngine OpUtils Detection");

  script_tag(name:"summary", value:"Detection of ManageEngine OpUtils.

The script sends a connection request to the server and attempts to detect ManageEngine OpUtils and to extract its
version.");
...
if ("OpUtils" >< res && "'info'>IP Address Management Software" >< res) {
  ...
  vers = eregmatch(pattern: "OpUtils<span>v ([0-9.]+)<", string: res);
  if (!isnull(vers[1]))
    version = vers[1];

  set_kb_item(name: "me_oputils/installed", value: TRUE);
