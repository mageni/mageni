...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2022.1330");
  ...
  script_name("Huawei EulerOS: Security Advisory for log4j (EulerOS-SA-2022-1330)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2022-1330");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2022-1330");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'log4j' package(s) announced via the EulerOS-SA-2022-1330 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"JMSSink in all versions of Log4j 1.x is vulnerable to deserialization of untrusted data when the attacker has write access to the Log4j configuration or if the configuration references an LDAP service the attacker has access to. The attacker can provide a TopicConnectionFactoryBindingName configuration causing JMSSink to perform JNDI requests that result in remote code execution in a similar fashion to CVE-2021-4104. Note this issue only affects Log4j 1.x when specifically configured to use JMSSink, which is not the default. Apache Log4j 1.2 reached end of life in August 2015. Users should upgrade to Log4j 2 as it addresses numerous other issues from the previous versions.(CVE-2022-23302)
...
if(release == "EULEROS-2.0SP5") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"log4j", rpm:"log4j~1.2.17~16.h4.eulerosv2r7", rls:"EULEROS-2.0SP5"))) {
    report += res;
  }
