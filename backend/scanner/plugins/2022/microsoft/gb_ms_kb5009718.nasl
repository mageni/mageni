...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.818945");
  ...
  script_name("Microsoft .NET Framework DoS Vulnerability (KB5009718)");

  script_tag(name:"summary", value:"This host is missing an important security
  update according to Microsoft KB5009718");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The flaw exists due to an error in .NET Framework which allows
  an unauthenticated attacker could cause a denial of service on an affected system.");
  ...
  script_xref(name:"URL", value:"https://support.microsoft.com/en-us/help/5009718");
      ...
      foreach item (registry_enum_keys(key:key))
      {
        ...
        if(NetPath && "\Microsoft.NET\Framework" >< NetPath)
        {
          foreach item (registry_enum_keys(key:key))
          {
            ...
            if(dllVer)
            {
              if(version_in_range(version:dllVer, test_version:"2.0.50727", test_version2:"2.0.50727.9049"))
              {
                vulnerable_range = "2.0.50727 - 2.0.50727.9049";
                break;
              }
              else if(version_in_range(version:dllVer, test_version:"4.7", test_version2:"4.7.3904"))
              {
