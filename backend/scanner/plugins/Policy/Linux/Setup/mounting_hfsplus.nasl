...
if(description)
{
  ...
  script_tag(name:"summary", value:"The hfsplus filesystem type is a replaces hfs that allows to
...
Note: This script looks for entry 'install hfsplus /bin/true' in files in /etc/modprobe.d/*.conf and
...
cmd = "lsmod; grep -r 'install hfsplus /bin/true' /etc/modprobe.d/*";
title = "Mounting of hfsplus filesystems";
solution = "Add or remove 'install hfsplus /bin/true' in config file (/etc/modprobe.d/*.conf) and run modprobe [-r] hfsplus";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux//etc/modprobe.d/ERROR") || get_kb_item("Policy/linux/lsmod/ERROR")){
  ...
  foreach file (get_kb_list("Policy/linux//etc/modprobe.d")){
    ...
    if(egrep(string:file_content, pattern:"^\s*install hfsplus /bin/true"))
      deactivated = TRUE;
  }
