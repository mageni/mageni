...
Note: This script looks for entry 'install cramfs /bin/true' in files in /etc/modprobe.d/*.conf and
...
cmd = "lsmod; grep -r 'install cramfs /bin/true' /etc/modprobe.d/*";
title = "Mounting of cramfs filesystems";
solution = "Add or remove 'install cramfs /bin/true' in config file (/etc/modprobe.d/*.conf) and run modprobe [-r] cramfs";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux//etc/modprobe.d/ERROR") || get_kb_item("Policy/linux/lsmod/ERROR")){
  ...
  foreach file (get_kb_list("Policy/linux//etc/modprobe.d")){
    ...
    if(egrep(string:file_content, pattern:"^\s*install cramfs /bin/true"))
      deactivated = TRUE;
  }
