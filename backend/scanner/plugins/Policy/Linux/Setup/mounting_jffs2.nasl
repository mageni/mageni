...
if(description)
{
  ...
  script_tag(name:"summary", value:"The journaling flash filesystem 2 filesystem type is a
...
Note: This script looks for entry 'install jffs2 /bin/true' in files in /etc/modprobe.d/*.conf and
...
solution = "Add or remove 'install jffs2 /bin/true' in config file (/etc/modprobe.d/*.conf) and run modprobe [-r] jffs2";
test_type = "SSH_Cmd";
...
if(get_kb_item("Policy/linux//etc/modprobe.d/ERROR") || get_kb_item("Policy/linux/lsmod/ERROR")){
  ...
  foreach file (get_kb_list("Policy/linux//etc/modprobe.d")){
    ...
    if(egrep(string:file_content, pattern:"^\s*install jffs2 /bin/true"))
      deactivated = TRUE;
  }
