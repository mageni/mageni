...
# $Id: win10_create_global_objects.nasl 10649 2018-07-27 07:16:55Z emoss $
#
# Check value for Create global objects (WMI)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312252");
  ...
  script_name('Microsoft Windows 10: Create global objects');
...
can create global objects that are available to all sessions. Users can still 
create objects that are specific to their own session if they do not have this 
user right.

A global object is an object that is created to be used by any number of 
processes or threads, even those not started within the users session. 
Remote Desktop Services uses global objects in its processes to facilitate 
...
select = 'AccountList';
keyname = 'SeCreateGlobalPrivilege';
