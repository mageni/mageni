...
# $Id: win10_perform_volume_maintenance_tasks.nasl 10649 2018-07-27 07:16:55Z emoss $
#
# Check value for Perform volume maintenance tasks (WMI)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312245");
  ...
  script_name('Microsoft Windows 10: Perform volume maintenance tasks');
...
can perform volume or disk management tasks, such as defragmenting an existing 
volume, creating or removing volumes, and running the Disk Cleanup tool.

Use caution when assigning this user right. Users with this user right can 
explore disks and extend files in to memory that contains other data. When the 
extended files are opened, the user might be able to read and modify the 
acquired data.");
  exit(0);
}
...
select = 'AccountList';
keyname = 'SeManageVolumePrivilege';
