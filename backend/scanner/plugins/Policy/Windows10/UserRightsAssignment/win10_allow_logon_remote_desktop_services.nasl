...
# $Id: win10_allow_logon_remote_desktop_services.nasl 10649 2018-07-27 07:16:55Z emoss $
#
# Check value for Allow log on through Remote Desktop Services (WMI)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312248");
...
or groups can access the logon screen of a remote device through a 
Remote Desktop Services connection. It is possible for a user to establish a 
Remote Desktop Services connection to a particular server but not be able to log 
on to the console of that same server.");
  exit(0);
}
