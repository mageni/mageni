...
# $Id: win10_deny_logon_as_service.nasl 10649 2018-07-27 07:16:55Z emoss $
#
# Check value for Deny log on as a service (WMI)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312257");
  ...
  script_name('Microsoft Windows 10: Deny log on as a service');
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (c) 2018 Greenbone Networks GmbH");
  script_family("Policy");
  script_dependencies("gb_wmi_access.nasl", "smb_reg_service_pack.nasl");
  script_add_preference(name:"Value", type:"entry", value:"Guests");
  script_mandatory_keys("Compliance/Launch");
  script_require_keys("WMI/access_successful");
  script_tag(name: "summary", value: "This policy setting determines which users 
are prevented from logging on to the service applications on a device.

A service is an application type that runs in the system background without a 
user interface. It provides core operating system features, such as web serving, 
event logging, file serving, printing, cryptography, and error reporting.");
  exit(0);
}
...
select = 'AccountList';
keyname = 'SeDenyServiceLogonRight';
