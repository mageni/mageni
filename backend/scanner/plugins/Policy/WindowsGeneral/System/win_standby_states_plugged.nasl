...
# $Id: win_standby_states_plugged.nasl 10819 2018-08-07 14:11:07Z emoss $
#
# Check value for Allow standby states (S1-S3) when sleeping (plugged in)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312612");
  ...
  script_name('Microsoft Windows: Allow standby states (S1-S3) when sleeping (plugged in)');
...
'Allow standby states (S1-S3) when sleeping (plugged in)' on Windows hosts (at least Windows 8.1).
...
title = 'Allow standby states (S1-S3) when sleeping (plugged in)';
fixtext = 'Set following UI path accordingly:
Computer Configuration/Administrative Templates/System/Power Management/Sleep Settings/' + title;
type = 'HKLM';
key = 'Software\\Policies\\Microsoft\\Power\\PowerSettings\\abfc2519-3608-4c2a-94ea-171b0ed546ab';
item = 'ACSettingIndex';
