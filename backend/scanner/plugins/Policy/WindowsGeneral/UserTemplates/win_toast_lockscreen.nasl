...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312500");
  ...
  script_name('Microsoft Windows: Turn off toast notifications on the lock screen');
...
'Turn off toast notifications on the lock screen' on Windows hosts (at least
Windows 8.1).

The setting specifies whether to show toast notifications on the lock screen.");
  exit(0);
}
...
foreach uid (uids){
  if(uid =~ "S-1-5-21-"){
      ...
      key = uid + '\\SOFTWARE\\Policies\\Microsoft\\Windows\\CurrentVersion\\PushNotifications';
