...
# $Id: win_rd_services.nasl 10989 2018-08-15 14:57:51Z emoss $
#
# Check value for Allow users to connect remotely by using Remote Desktop
# Services
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312419");
  ...
  script_name('Microsoft Windows: Allow users to connect remotely (Remote Desktop Services)');
...
'Allow users to connect remotely by using Remote Desktop Services' on Windows
hosts (at least Windows 7).

The setting configures remote access to computers by using Remote Desktop Services.");
  exit(0);
}
