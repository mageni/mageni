...
# $Id: win_connect_now.nasl 10809 2018-08-07 11:19:51Z emoss $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.312569");
  ...
  script_name('Microsoft Windows: Configuration of wireless settings using Windows Connect Now');
...
'Configuration of wireless settings using Windows Connect Now' on Windows hosts
(at least Windows 7).

The policy setting allows the configuration of wireless settings using Windows
Connect Now (WCN). The WCN Registrar enables the discovery and configuration of
devices over Ethernet (UPnP), over In-band 802.11 WLAN, through the Windows
Portable Device API (WPD), and via USB Flash drives.");
  exit(0);
}
