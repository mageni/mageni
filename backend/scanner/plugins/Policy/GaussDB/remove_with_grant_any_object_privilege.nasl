...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.115006");
  script_version("2020-07-29T09:51:47+0000");
  script_tag(name:"last_modification", value:"2020-07-30 10:23:02 +0000 (Thu, 30 Jul 2020)");
  script_tag(name:"creation_date", value:"2020-04-06 06:54:28 +0000 (Mon, 06 Apr 2020)");
  script_tag(name:"cvss_base", value:"0.0");
  script_tag(name:"cvss_base_vector", value:"AV:L/AC:H/Au:S/C:N/I:N/A:N");
  script_tag(name:"qod", value:"97");

  script_name("ZSQL: Check for users with GRANT ANY OBJECT PRIVILEGE permission");
  ...
  script_tag(name:"summary", value:"Searches for users with GRANT ANY OBJECT PRIVILEGE permission
and checks whether they are authorized to have it. Users with this permission can grant any object
permission to any user. Therefore, grant this permission only when absolutely necessary.");
...
cmd = "SELECT USERNAME, PRIVILEGE FROM DB_USERS_SYS_PRIVS WHERE USERNAME = 'user_name' AND PRIVILEGE = 'GRANT ANY OBJECT PRIVILEGE';";
title = "List users with GRANT ANY OBJECT PRIVILEGE permission";
solution ="1) Revoke GRANT ANY OBJECT PRIVILEGE FROM user_name;
2) Revoke GRANT ANY OBJECT PRIVILEGE FROM role_name;";
test_type = "SQL_Query";
...
if(get_kb_item("Policy/zsql/zsql_db_user_sys_privs/ssh/ERROR") || get_kb_item("Policy/zsql/zsql_role_sys_privs/ssh/ERROR")){
  ...
  foreach key1(keys(grantee_list)){
    if( key1 =~ "GRANT ANY OBJECT PRIVILEGE"){
  ...
  foreach key2(keys(role_sys_privs_list)){
    if(key2 =~ "GRANT ANY OBJECT PRIVILEGE"){
