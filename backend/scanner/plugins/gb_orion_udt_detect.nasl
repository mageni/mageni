...
# $Id: gb_orion_udt_detect.nasl 10929 2018-08-11 11:39:44Z cfischer $
#
...
if (description)
{
  ...
  script_name("SolarWinds Orion User Device Tracker Detection");
  ...
  script_tag(name:"summary", value:"Checks for the presence of SolarWinds Orion User Device Tracker.");
...
if ("SolarWinds Platform" >< buf || "SolarWinds Orion" >< buf || "Orion Platform" >< buf) {
  udt = eregmatch(string:buf, pattern: "UDT ([0-9.]+)", icase:TRUE);
  if (!isnull(udt)) {
    vers = string("unknown");
    if (!isnull(udt[1])) {
      vers=chomp(udt[1]);
    ...
    set_kb_item(name: string("www/", port, "/orion_udt"), value: vers);
    set_kb_item(name: "orion_udt/installed", value: TRUE);

    cpe = build_cpe(value: vers, exp:"^([0-9.]+)", base: "cpe:/a:solarwinds:user_device_tracker:");
    if (isnull(cpe))
      cpe = 'cpe:/a:solarwinds:user_device_tracker';

    register_product(cpe:cpe, location:dir, port:port);

    log_message(data: build_detection_report(app:"SolarWinds User Device Tracker", version: vers,
                                             install: dir, cpe: cpe,
                                             concluded: udt[0]), port:port);
  }
}
