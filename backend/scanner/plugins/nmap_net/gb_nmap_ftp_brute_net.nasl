...
# $Id: gb_nmap_ftp_brute_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104102");
  ...
  script_name("Nmap NSE net: ftp-brute");
  ...
  script_tag(name:"summary", value:"Tries to get FTP login credentials by guessing usernames and passwords.
...
if (phase == 1) {
    ...
    if (!isnull(pref) && pref != "") {
        argv["passlimit"] = string('"', pref, '"');
    }
    pref = script_get_preference("userlimit");
    if (!isnull(pref) && pref != "") {
        argv["userlimit"] = string('"', pref, '"');
    }
    ...
    nmap_nse_register(script:"ftp-brute", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"ftp-brute");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (ftp-brute.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
