...
# $Id: gb_nmap_ftp_anon_net.nasl 12117 2018-10-26 10:50:36Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.104031");
  ...
  script_name("Nmap NSE net: ftp-anon");
...
if (phase == 1) {
    ...
    if (!isnull(pref) && pref != "") {
        argv["ftp-anon.maxlist"] = string('"', pref, '"');
    }
    nmap_nse_register(script:"ftp-anon", args:argv);
} else if (phase == 2) {
    res = nmap_nse_get_results(script:"ftp-anon");
    foreach portspec (keys(res)) {
        output_banner = 'Result found by Nmap Security Scanner (ftp-anon.nse) http://nmap.org:\n\n';
        if (portspec == "0") {
