...
if(description)
{
  ...
  script_name("Advantech WISE-PaaS Dashboard Version Detection (Windows)");
  ...
  of Advantech WISE-PaaS Dashboard for Windows.");
...
foreach key ( key_list ) {
  foreach item ( registry_enum_keys( key:key ) ) {
    ...
    if( !appName || appName !~ "WISE-PaaS_Dashboard [a-z 0-9.]+" )
      continue;
    ...
    set_kb_item( name:"advantech/wise_paas_dashboard/win/detected", value:TRUE );

    register_and_report_cpe( app:pub + " " + appName, ver:version, concluded:concluded,
                             base:"cpe:/a:advantech:wise_paas_dashboard:", expr:"^([0-9.]+)", insloc:location, regService:"smb-login", regPort:0 );
    exit( 0 );
  }
}
