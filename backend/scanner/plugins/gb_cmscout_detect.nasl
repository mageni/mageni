...
# $Id: gb_cmscout_detect.nasl 10894 2018-08-10 13:09:25Z cfischer $
#
...
if(description)
{
  ...
  script_tag(name:"summary", value:"This script finds the installed CMScout version and saves
  the result in KB.");
...
foreach dir( make_list_unique( "/", "/cmscout", "/cmscout", cgi_dirs( port:port ) ) ) {
  ...
  if( rcvRes =~ "HTTP/1.. 200" && ( ":: CMScout ::" >< rcvRes || ">Welcome to CMScout" >< rcvRes ) ) {

    version = "unknown";

    ver = eregmatch( pattern:"CMScout ([0-9.]+)", string:rcvRes );
