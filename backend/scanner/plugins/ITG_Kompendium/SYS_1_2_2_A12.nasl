...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150020");
...
Die Kern-Anforderung 'A12: Redundanz und Hochverfuegbarkeit' beschreibt, dass bestimmte
Betriebssystemfunktionen je nach Verfuegbarkeitsanforderung  umgesetzt werden sollten.");
...
title = "Redundanz und Hochverfuegbarkeit";

if (host_runs("windows_server_2012") != "yes"){
  result = itg_result_wrong_target();
  desc = itg_desc_wrong_target();
  itg_set_kb_entries(result:result, desc:desc, title:title, id:"SYS.1.2.2.A12");
  exit(0);
}
