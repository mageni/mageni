...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150594");
  ...
  script_tag(name:"summary", value:"Der Client MUSS zeitnah rebootet werden, nachdem der Kernel des
Betriebssystems aktualisiert wurde. Ist dies nicht moeglich, MUSS alternativ Live-Patching des
Kernels aktiviert werden.");
...
title = "Kernel-Aktualisierungen auf unixartigen Systemen";

if (host_runs("linux") != "yes"){
  result = itg_result_wrong_target();
  desc = itg_desc_wrong_target();
  itg_set_kb_entries(result:result, desc:desc, title:title, id:"SYS.2.3.A4");
  exit(0);
}
