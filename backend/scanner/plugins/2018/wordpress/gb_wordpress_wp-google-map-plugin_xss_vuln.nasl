...
# $Id: gb_wordpress_wp-google-map-plugin_xss_vuln.nasl 11156 2018-08-29 09:25:17Z asteins $
#
...
if (description)
{
  ...
  script_tag(name:"solution", value:"Upgrade to version 4.0.4 or later.");
...
res = http_get_cache(port: port, item: dir + "/wp-content/plugins/wp-google-map-plugin/readme.txt");

if ("WP Google Map Plugin" >< res && "flippercode" >< res && "Changelog" >< res) {

  changelog = eregmatch(pattern: "== Changelog(.*)", string: res);

  vers = eregmatch(pattern: "= ([0-9.]+) =", string: changelog);
  if (isnull(vers[1]))
    vers = eregmatch(pattern: "Version ([0-9.]+)", string: changelog);

  if (!isnull(vers[1]) && version_is_less(version: vers[1], test_version: "4.0.4")) {
    report = report_fixed_ver(installed_version: vers[1], fixed_version: "4.0.4");
