...
# $Id: gb_wordpress_events-manager_xss_vuln.nasl 11156 2018-08-29 09:25:17Z asteins $
#
...
if (description)
{
  ...
  script_tag(name:"solution", value:"Upgrade to version 5.9 or later.");
...
res = http_get_cache(port: port, item: dir + "/wp-content/plugins/events-manager/readme.txt");

if ("=== Events Manager ===" >< res && "Changelog" >< res) {

  vers = eregmatch(pattern: "Stable tag: ([0-9.]+)", string: res);

  if (!isnull(vers[1]) && version_is_less(version: vers[1], test_version: "5.9")) {
    report = report_fixed_ver(installed_version: vers[1], fixed_version: "5.9");
