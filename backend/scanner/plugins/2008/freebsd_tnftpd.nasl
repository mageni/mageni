...
CVE-2004-0794
Multiple signal handler race conditions in lukemftpd (aka tnftpd
before 20040810) allow remote authenticated attackers to cause a
...
if(description)
{
 script_oid("1.3.6.1.4.1.25623.1.0.300604");
 ...
 script_name("FreeBSD Ports: tnftpd");
...
txt = "";
vuln = 0;
bver = portver(pkg:"tnftpd");
if(!isnull(bver) && revcomp(a:bver, b:"20040810")<0) {
    txt += 'Package tnftpd version ' + bver + ' is installed which is known to be vulnerable.\n';
    vuln = 1;
}
bver = portver(pkg:"lukemftpd");
if(!isnull(bver) && revcomp(a:bver, b:"0")>=0) {
    txt += 'Package lukemftpd version ' + bver + ' is installed which is known to be vulnerable.\n';
    vuln = 1;
}
