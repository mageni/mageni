...
tag_insight = "Several vulnerabilities have been found in xine-ui and xine-lib,
potentially allowing an attacker to overwrite files with the rights of the
user.";
tag_solution = "All users of xine-ui or another xine-based player should upgrade to the
...
if(description)
{
 script_oid("1.3.6.1.4.1.25623.1.0.300432");
 ...
 script_name("Gentoo Security Advisory GLSA 200404-20 (xine)");
...
res = "";
report = "";
if ((res = ispkgvuln(pkg:"media-video/xine-ui", unaffected: make_list("ge 0.9.23-r2"), vulnerable: make_list("le 0.9.23-r1"))) != NULL) {
    report += res;
}
if ((res = ispkgvuln(pkg:"media-libs/xine-lib", unaffected: make_list("ge 1_rc3-r3"), vulnerable: make_list("le 1_rc3-r2"))) != NULL) {
    report += res;
}
