...
# $Id: gb_avast_endpoint_protection_suite_plus_detect.nasl 11279 2018-09-07 09:08:31Z cfischer $
#
...
if(description)
{
  ...
  The script logs in via smb, searches for string 'Avast Endpoint Protection Suite
  Plus' in the registry and reads the version information from registry.");
...
foreach item (registry_enum_keys(key:key))
{
  ...
  if("avast! Endpoint Protection Suite Plus" >< avastName)
  {
    ...
    if(avastVer)
    {
      set_kb_item(name:"Avast/Endpoint-Protection-Suite-Plus/Win/Ver", value:avastVer);

      cpe = build_cpe(value:avastVer, exp:"^([0-9.]+)", base:"cpe:/a:avast:endpoint_protection_suite_plus:");
      if(isnull(cpe))
        cpe = "cpe:/a:avast:endpoint_protection_suite_plus";

      register_product(cpe:cpe, location:avastPath);

      log_message(data: build_detection_report(app: "Avast Endpoint Protection Suite Plus",
                                               version: avastVer,
                                               install: avastPath,
                                               cpe: cpe,
                                               concluded: avastVer));
