...
foreach item (registry_enum_keys(key:key))
{
  ...
  if("avast! Endpoint Protection Plus" >< avastName)
  {
    ...
    if(avastVer)
    {
      set_kb_item(name:"Avast/Endpoint-Protection-Plus/Win/Ver", value:avastVer);

      cpe = build_cpe(value:avastVer, exp:"^([0-9.]+)", base:"cpe:/a:avast:endpoint_protection_plus:");
      if(isnull(cpe))
        cpe = "cpe:/a:avast:endpoint_protection_plus";

      register_product(cpe:cpe, location:avastPath);

      log_message(data: build_detection_report(app: "Avast Endpoint Protection Plus",
                                               version: avastVer,
                                               install: avastPath,
                                               cpe: cpe,
                                               concluded: avastVer));
