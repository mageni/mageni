...
# $Id: gb_nmap_smb_enum_domains.nasl 12115 2018-10-26 09:30:41Z cfischer $
#
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.801296");
  ...
  script_tag(name:"summary", value:"This script attempts to enumerate domains on a Windows system,
  along with their policies.

  This is a wrapper on the Nmap Security Scanner's smb-enum-domains.nse.");
...
argv = make_list("nmap", "--script=smb-enum-domains.nse", "-p", port, get_host_ip());
...
if(res)
{
  ...
  if("smb-enum-domains" >< result) {
    msg = string('Result found by Nmap Security Scanner (smb-enum-domains.nse) ',
                'http://nmap.org:\n\n', result);
