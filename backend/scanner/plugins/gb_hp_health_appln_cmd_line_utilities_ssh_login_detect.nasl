...
if(description)
{
  ...
  script_name("HP System Health Application and Command Line Utilities Detection (Linux SSH Login)");
  ...
  script_tag(name:"summary", value:"SSH login-based detection of the HP System Health Application
...
if(!rpms || rpms !~ ";hp-health~")
  exit(0);

set_kb_item(name:"hp/system_health_and_clu/detected", value:TRUE);
set_kb_item(name:"hp/system_health_and_clu/ssh-login/detected", value:TRUE);

version = "unknown";
path = "/opt/hp/hp-health/";

# e.g.
# hp-health~10.40~1777.17.rhel7~x86_64
vers = eregmatch(pattern:";hp-health~([0-9.]+)[^;]*", string:rpms);
if(vers[1]) {
  version = vers[1];
...
cpe = build_cpe(value:version, exp:"^([0-9.]+)", base:"cpe:/a:hp:system_health_application_and_command_line_utilities:");
if(!cpe)
