...
if( ! port = get_kb_item( "hsts/includeSubDomains/missing/port" ) ) exit( 0 );

banner = get_kb_item( "hsts/" + port + "/banner" );

log_message( port:port, data:'The remote HTTPS Server is missing the "includeSubDomains" attribute in the HSTS header:' + banner );
exit( 0 );
