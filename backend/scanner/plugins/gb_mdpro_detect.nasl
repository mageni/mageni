...
if(description)
{
  ...
  script_tag(name:"summary", value:"This script detects the installed version of MDPro and sets
  the result in KB.");
...
foreach dir( make_list_unique( "/mdpro", "/md", cgi_dirs( port:port ) ) ) {
  ...
  if( rcvRes =~ "HTTP/1.. 200" && egrep( pattern:"MD-?Pro", string:rcvRes ) ) {
    ...
    log_message( data: build_detection_report( app:"MDPro",
                                               version:version,
                                               install:install,
                                               cpe:cpe,
                                               concluded:ver[0] ),
