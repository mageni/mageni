...
if ("SolarWinds Platform" >< buf || "SolarWinds Orion" >< buf || "Orion Platform" >< buf) {
  ...
  if (!isnull(ipam)) {
    ...
    set_kb_item(name: string("www/", port, "/orion_ipam"), value: vers);
    set_kb_item(name: "orion_ipam/installed", value: TRUE);
    ...
    log_message(data: build_detection_report(app:"SolarWinds IP Address Manager", version: vers,
                                             install: dir, cpe: cpe,
                                             concluded: ipam[0]), port:port);
  }
}
